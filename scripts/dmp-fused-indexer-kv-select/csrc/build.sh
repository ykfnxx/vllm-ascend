...
function help_info() {
    ...
    echo "                     For example: -c \"ascend910_93\" or -c \"ascend910b\". ascend910b1 is normalized to ascend910b."
    echo
...
function normalize_compute_unit()
{
    local raw_compute_unit="$1"
    local normalized_compute_unit=""
    local unit
    local -a compute_units

    IFS=';' read -ra compute_units <<< "${raw_compute_unit,,}"
    for unit in "${compute_units[@]}"; do
        case "${unit}" in
        ascend910_9391)
            unit="ascend910_93"
            ;;
        ...
        if [ -z "${normalized_compute_unit}" ]; then
            normalized_compute_unit="${unit}"
        else
            normalized_compute_unit="${normalized_compute_unit};${unit}"
        fi
    done

    echo "${normalized_compute_unit}"
}
...
if [ -n "${ascend_compute_unit}" ];then
    ascend_compute_unit="$(normalize_compute_unit "${ascend_compute_unit}")"
