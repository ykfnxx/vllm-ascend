...
/*!
 * \file lightning_indexer_decode.cpp
 * \brief
 */

#include "kernel_operator.h"
#include "lib/matmul_intf.h"
#include "lightning_indexer_decode_template_tiling_key.h"
#include "lightning_indexer_decode_kernel.h"
...
#define INVOKE_LI_NO_KFC_OP_IMPL(templateClass, ...)                                                                   \
    do {                                                                                                               \
        ...
        LI_COPY_TILING_DATA(LIDecodeTilingData, tiling);                                                                     \
        op.Init(query, key, weights, actualSeqLengths, blocktable, sparseIndices, user,                             \
                tiling_data, &tPipe);                                                                                  \
        op.Process();                                                                                                  \
    } while (0)
...
template <int DT>
__global__ __aicore__ void lightning_indexer_decode(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
