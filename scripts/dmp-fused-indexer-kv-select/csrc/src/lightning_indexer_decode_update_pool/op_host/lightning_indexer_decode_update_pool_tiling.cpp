...
#include "lightning_indexer_decode_update_pool_tiling.h"
#include <algorithm>
#include "../op_kernel/lightning_indexer_decode_update_pool_template_tiling_key.h"
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::GetNpuInfo(LIUpdatePoolTilingInfo &tilingInfo) const
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("LightningIndexerDecodeUpdatePool", "opName got from TilingContext is nullptr.");
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::GetTensorInfo(LIUpdatePoolTilingInfo &tilingInfo) const
{
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::CheckDtype(const LIUpdatePoolTilingInfo &tilingInfo) const
{
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::CheckShape(LIUpdatePoolTilingInfo &tilingInfo) const
{
    const auto &op = tilingInfo.opParamInfo;
    ...
    OPS_ERR_IF(reqPoolShape.GetDimNum() != DIM_NUM_ONE,
               OPS_LOG_E(tilingInfo.opName, "req_pool_entries must be [B]."), return ge::GRAPH_FAILED);
    OPS_ERR_IF(cacheShape.GetDimNum() != DIM_NUM_TWO,
               OPS_LOG_E(tilingInfo.opName, "cache_slots must be [pool_size, 262144]."),
               return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(reqPoolShape.GetShapeSize() != tilingInfo.bSize ||
               ...
               OPS_LOG_E(tilingInfo.opName,
                         "query batch, req_pool_entries, actual_seq_lengths_key, and block_table must match."),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(tilingInfo.poolSize == 0 || cacheShape.GetDim(1) != CACHE_SLOTS_SIZE,
               OPS_LOG_E(tilingInfo.opName, "cache_slots must have shape [pool_size, 262144] with pool_size > 0."),
               return ge::GRAPH_FAILED);
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::ParseAndCheck(LIUpdatePoolTilingInfo &tilingInfo)
{
...
ge::graphStatus LightningIndexerDecodeUpdatePoolTiling::DoTiling(LIUpdatePoolTilingInfo *tilingInfo)
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(tilingInfo->platformInfo);
    uint32_t aivNum = ascendcPlatform.GetCoreNumAiv();
    uint32_t aicNum = ascendcPlatform.GetCoreNumAic();
...
static ge::graphStatus TilingPrepareForLightningIndexerDecodeUpdatePool(gert::TilingParseContext * /* context */)
{
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingForLightningIndexerDecodeUpdatePool(gert::TilingContext *context)
{
    OPS_ERR_IF(context == nullptr,
               OPS_REPORT_VECTOR_INNER_ERR("LightningIndexerDecodeUpdatePool", "Tiling context is null."),
               return ge::GRAPH_FAILED);
    LIUpdatePoolTilingInfo liInfo;
    LightningIndexerDecodeUpdatePoolTiling liTiling(context);
...
IMPL_OP_OPTILING(LightningIndexerDecodeUpdatePool)
    .Tiling(TilingForLightningIndexerDecodeUpdatePool)
    .TilingParse<LIUpdatePoolCompileInfo>(TilingPrepareForLightningIndexerDecodeUpdatePool);
