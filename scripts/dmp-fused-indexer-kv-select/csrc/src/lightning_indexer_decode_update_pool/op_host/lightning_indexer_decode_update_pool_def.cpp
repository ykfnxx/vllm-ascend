...
namespace ops {
class LightningIndexerDecodeUpdatePool : public OpDef {
public:
    explicit LightningIndexerDecodeUpdatePool(const char *name) : OpDef(name)
    {
        this->Input("query")
            .ParamType(REQUIRED)
...
OP_ADD(LightningIndexerDecodeUpdatePool);
} // namespace ops
