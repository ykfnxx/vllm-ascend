...
private:
    ...
    __aicore__ inline uint32_t CopyDecodedPayloadOut(int64_t outOffset, const LocalTensor<uint32_t> &payloadLocal,
                                                     ...
                                                     int64_t outputCount, uint32_t validCount,
                                                     bool mayHaveInvalid);
                                                                ...
                                                                bool mayHaveInvalid)
{
    if (mayHaveInvalid) {
        ...
        PipeBarrier<PIPE_V>();
        DecodeIndexFromPayload(indexLocal.template ReinterpretCast<uint32_t>(), payloadLocal, validCount);
    } else {
    ...
    DataCopyPad(topkIndexGm[outOffset], indexLocal,
                {1, static_cast<uint16_t>(outputCount * sizeof(int32_t)), 0, 0});
    SetWaitFlag<HardEvent::MTE3_V>(HardEvent::MTE3_V);

    if (mayHaveInvalid) {
        Duplicate(slotLocal, LICommon::ConstInfo::INVALID_IDX, BASE_TOPK);
        PipeBarrier<PIPE_V>();
        DecodeSlotFromPayload(slotLocal.template ReinterpretCast<uint32_t>(), payloadLocal, scratchLocal, validCount);
    } else {
        DecodeSlotFromPayload(slotLocal.template ReinterpretCast<uint32_t>(), payloadLocal, scratchLocal, BASE_TOPK);
    }
    SetWaitFlag<HardEvent::V_S>(HardEvent::V_S);
    uint32_t missCount = 0;
    while (missCount < validCount &&
...
__aicore__ inline void LIVector<LIT>::ProcessVec(const LICommon::RunInfo &info)
{
    ...
    if (info.isLastS2InnerLoop) {
        // Padding and cache misses both use slot14=0x3fff. Keep short-row padding behind valid tokens.
        if (info.actS2Size >= BASE_TOPK) {
