...
def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark the current LightningIndexerDecode operator.")
...
def validate_args(args):
    ...
    if args.block_size != 128:
        raise ValueError("LightningIndexerDecode PA_BSND performance comparison requires block_size=128.")
    if args.warmup < 0 or args.iters <= 0:
        raise ValueError("warmup must be non-negative and iters must be positive.")


def import_indexer_decode():
    import torch
...
def run_indexer_decode(op, tensors):
    output = op(
        tensors["query"],
        tensors["key"],
        tensors["weights"],
    ...
    if isinstance(output, (tuple, list)):
        if len(output) != 1:
            raise RuntimeError(f"Expected one LightningIndexerDecode output, got {len(output)}.")
        output = output[0]
    return output


def benchmark_case(torch, op, tensors, bs, seqlen, warmup, iters):
    output = run_indexer_decode(op, tensors)
    ...
    if int(output.min().item()) < 0 or int(output.max().item()) >= seqlen:
        raise RuntimeError("LightningIndexerDecode returned an out-of-range token index.")
...
def main():
    args = parse_args()
    validate_args(args)
    torch, op = import_indexer_decode()
    ...
    for bs in args.batch_sizes:
        for seqlen in args.seqlens:
            ...
            print(f"summary op=LightningIndexerDecode bs={bs} seqlen={seqlen} avg_us={avg_us}")
            del tensors
            torch.npu.empty_cache()

    print("\nLightningIndexerDecode")
