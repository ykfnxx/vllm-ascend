...
constexpr int64_t DECODE_UPDATE_SPARSE_COUNT = 2048;
constexpr int64_t DECODE_UPDATE_CACHE_SLOTS_SIZE = 262144;

std::tuple<at::Tensor, at::Tensor, at::Tensor> MakeLightningIndexerDecodeUpdateOutput(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &weights, const at::Tensor &cacheSlots,
    const at::Tensor &actualSeqLengthsKey, const at::Tensor &blockTable)
{
    ...
    TORCH_CHECK(cacheSlots.dim() == 2, "cache_slots must have shape [B, 262144].");
    ...
    TORCH_CHECK(query.size(0) == cacheSlots.size(0), "query and cache_slots batch dim must match.");
    ...
    TORCH_CHECK(cacheSlots.size(1) == DECODE_UPDATE_CACHE_SLOTS_SIZE,
        "cache_slots must have shape [B, 262144].");
    ...
    auto topkIndex = at::empty({query.size(0), key.size(2), DECODE_UPDATE_SPARSE_COUNT},
        query.options().dtype(at::kInt));
    auto topkSlots = at::empty({query.size(0), key.size(2), DECODE_UPDATE_SPARSE_COUNT},
        query.options().dtype(at::kInt));
    auto missCount = at::empty({query.size(0)}, query.options().dtype(at::kInt));
    return std::make_tuple(topkIndex, topkSlots, missCount);
}

std::tuple<at::Tensor, at::Tensor, at::Tensor> npu_lightning_indexer_decode_update_npu(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &weights, at::Tensor &cacheSlots,
    const at::Tensor &actualSeqLengthsKey, const at::Tensor &blockTable)
{
    auto outputs = MakeLightningIndexerDecodeUpdateOutput(
        query, key, weights, cacheSlots, actualSeqLengthsKey, blockTable);
    auto &topkIndex = std::get<0>(outputs);
    auto &topkSlots = std::get<1>(outputs);
    auto &missCount = std::get<2>(outputs);
    EXEC_NPU_CMD_v0(aclnnLightningIndexerDecodeUpdate, query, key, weights, cacheSlots, actualSeqLengthsKey,
        blockTable, topkIndex, topkSlots, missCount);
    return outputs;
}

std::tuple<at::Tensor, at::Tensor, at::Tensor> npu_lightning_indexer_decode_update_meta(
    const at::Tensor &query, const at::Tensor &key, const at::Tensor &weights, at::Tensor &cacheSlots,
    const at::Tensor &actualSeqLengthsKey, const at::Tensor &blockTable)
{
    return MakeLightningIndexerDecodeUpdateOutput(query, key, weights, cacheSlots, actualSeqLengthsKey, blockTable);
}
...
{
    m.impl("npu_lightning_indexer_decode_update", &custom::npu_lightning_indexer_decode_update_npu);
}

TORCH_LIBRARY_IMPL(custom, Meta, m)
{
    m.impl("npu_lightning_indexer_decode_update", &custom::npu_lightning_indexer_decode_update_meta);
}
