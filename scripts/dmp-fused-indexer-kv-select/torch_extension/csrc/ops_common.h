...
// while newer headers may refer to the ACL_* aliases.
...
static void append_unique_path(std::vector<std::string> &path_list, const std::string &path)
{
    if (path.empty()) {
        return;
    }
    std::string real_path_str = real_path(path);
    if (real_path_str.empty()) {
        return;
    }
    if (real_path_str.back() != '/') {
        real_path_str += "/";
    }
    for (const auto &item : path_list) {
        if (item == real_path_str) {
            return;
        }
    }
    path_list.push_back(real_path_str);
}

static void append_custom_opapi_lib_dir(std::vector<std::string> &path_list, const std::string &vendor_path)
{
    std::string lib_dir = vendor_path + "/op_api/lib/";
    std::string lib_path = lib_dir + "/libcust_opapi.so";
    if (!is_file_exist(lib_path)) {
        return;
    }
    append_unique_path(path_list, lib_dir);
}

static void append_custom_vendor_from_opp(std::vector<std::string> &path_list, const std::string &opp_path)
{
    append_custom_opapi_lib_dir(path_list, opp_path + "/vendors/customize");
}
...
inline std::vector<std::string> get_custom_lib_path()
{
    ...
    if (ascend_custom_opppath == nullptr) {
        ...
        auto custom_vendor_list = split_str(ascend_custom_opppath_str, ":");
        for (auto &it : custom_vendor_list) {
            append_custom_opapi_lib_dir(custom_lib_path_list, it);
        }
    }

    char *lightning_indexer_decode_install_opp_path = std::getenv("LIGHTNING_INDEXER_DECODE_INSTALL_OPP_PATH");
    if (lightning_indexer_decode_install_opp_path != nullptr) {
        append_custom_vendor_from_opp(custom_lib_path_list, lightning_indexer_decode_install_opp_path);
    }

    char *ascend_opp_path = std::getenv("ASCEND_OPP_PATH");
    if (ascend_opp_path != nullptr) {
        append_custom_vendor_from_opp(custom_lib_path_list, ascend_opp_path);
    }

    char *ascend_home_path = std::getenv("ASCEND_HOME_PATH");
    if (ascend_home_path != nullptr) {
        append_custom_vendor_from_opp(custom_lib_path_list, ascend_home_path);
        append_custom_vendor_from_opp(custom_lib_path_list, std::string(ascend_home_path) + "/opp");
    }

    append_custom_vendor_from_opp(custom_lib_path_list, "/usr/local/Ascend/ascend-toolkit/latest/opp");
    append_custom_vendor_from_opp(custom_lib_path_list, "/usr/local/Ascend/latest/opp");
