...
inline std::vector<std::string> get_custom_lib_path()
{
    ...
    char *lookup_maintain_install_opp_path = std::getenv("DMP_LOOKUP_MAINTAIN_INSTALL_OPP_PATH");
    if (lookup_maintain_install_opp_path != nullptr) {
        append_custom_vendor_from_opp(custom_lib_path_list, lookup_maintain_install_opp_path);
    }
