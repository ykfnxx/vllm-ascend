...
if [[ "$VLLM_ASCEND_DMP_SERIALIZE_MAINTAIN" != "0" && \
      "$VLLM_ASCEND_DMP_SERIALIZE_MAINTAIN" != "1" ]]; then
    echo "VLLM_ASCEND_DMP_SERIALIZE_MAINTAIN must be 0 or 1." >&2
    return 1 2>/dev/null || exit 1
fi
if [[ "$VLLM_ASCEND_DMP_SERIALIZE_MAINTAIN" == "1" && \
      "$VLLM_ASCEND_ENABLE_DMP_LOOKUP_MAINTAIN" != "1" ]]; then
    echo "Serial Maintain requires DMP Lookup/Maintain mode." >&2
    return 1 2>/dev/null || exit 1
fi
