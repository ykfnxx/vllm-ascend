...
constexpr int32_t KV_GATHER_FULL_K_ROPE_IDX = 6;
constexpr int32_t KV_GATHER_FULL_KV_CACHE_IDX = 7;
constexpr int32_t KV_GATHER_FULL_KV_BLOCK_TABLE_IDX = 8;
constexpr int32_t KV_GATHER_FULL_KV_ACTSEQ_IDX = 9;
constexpr int32_t KV_GATHER_FULL_Q_ACTSEQ_IDX = 10;
...
int32_t GatherSelectionKvCacheTiling::FullKRopeIdx() const
{
    return opKind_ == GatherSelectionKvCacheTilingOp::KV_GATHER ? KV_GATHER_FULL_K_ROPE_IDX : FULL_K_ROPE_IDX;
}

int32_t GatherSelectionKvCacheTiling::FullKvCacheIdx() const
{
    return opKind_ == GatherSelectionKvCacheTilingOp::KV_GATHER ? KV_GATHER_FULL_KV_CACHE_IDX : FULL_KV_CACHE_IDX;
}

int32_t GatherSelectionKvCacheTiling::FullKvBlockTableIdx() const
{
    return opKind_ == GatherSelectionKvCacheTilingOp::KV_GATHER
        ? KV_GATHER_FULL_KV_BLOCK_TABLE_IDX
        : FULL_KV_BLOCK_TABLE_IDX;
}

int32_t GatherSelectionKvCacheTiling::FullKvActualSeqIdx() const
{
    return opKind_ == GatherSelectionKvCacheTilingOp::KV_GATHER ? KV_GATHER_FULL_KV_ACTSEQ_IDX : FULL_KV_ACTSEQ_IDX;
}

int32_t GatherSelectionKvCacheTiling::FullQActualSeqIdx() const
{
    return opKind_ == GatherSelectionKvCacheTilingOp::KV_GATHER ? KV_GATHER_FULL_Q_ACTSEQ_IDX : FULL_Q_ACTSEQ_IDX;
}
...
ge::graphStatus GatherSelectionKvCacheTiling::GetFullKvCacheShape()
{
    ...
    auto fulKRopeIn = context_->GetInputShape(FullKRopeIdx());
    ...
    auto fulKvCacheIn = context_->GetInputShape(FullKvCacheIdx());
...
ge::graphStatus GatherSelectionKvCacheTiling::GetFullKvBlkTable()
{
    ...
    auto fulKvBlkTIn = context_->GetInputShape(FullKvBlockTableIdx());
...
ge::graphStatus GatherSelectionKvCacheTiling::GetSeqLenIn()
{
    ...
    auto fulKvSeqIn = context_->GetInputShape(FullKvActualSeqIdx());
    ...
    auto fulQSeqIn = context_->GetInputShape(FullQActualSeqIdx());
...
ge::graphStatus GatherSelectionKvCacheTiling::GetInputDtypeInfo()
{
    ...
    auto fKRopeDesc = context_->GetInputDesc(FullKRopeIdx());
    ...
    auto fKvCacheDesc = context_->GetInputDesc(FullKvCacheIdx());
    ...
    auto fKvBTDesc = context_->GetInputDesc(FullKvBlockTableIdx());
    ...
    auto fKvActSeqDesc = context_->GetInputDesc(FullKvActualSeqIdx());
    ...
    auto fQActSeqDesc = context_->GetInputDesc(FullQActualSeqIdx());
...
ge::graphStatus Tiling4GatherSelectionKvCache(gert::TilingContext* context)
{
    ...
    GatherSelectionKvCacheTiling tiling(
        context, GatherSelectionKvCacheTilingOp::GATHER_SELECTION_KV_CACHE);
    return tiling.RunTiling();
}

ge::graphStatus Tiling4KVSelect(gert::TilingContext* context)
{
    OPS_LOG_I(context->GetNodeName(), "TilingForKVSelect running.");
    GatherSelectionKvCacheTiling tiling(
        context, GatherSelectionKvCacheTilingOp::KV_SELECT);
...
ge::graphStatus Tiling4KVGather(gert::TilingContext* context)
{
    OPS_LOG_I(context->GetNodeName(), "TilingForKVGather running.");
    GatherSelectionKvCacheTiling tiling(
        context, GatherSelectionKvCacheTilingOp::KV_GATHER);
    ge::graphStatus ret = tiling.RunTiling();
    if (ret == ge::GRAPH_SUCCESS) {
        context->SetTilingKey(0);
    }
...
IMPL_OP_OPTILING(KVSelect)
    .Tiling(Tiling4KVSelect)
    .TilingParse<GatherSelectionKvCacheCompileInfo>(TilingPrepare4GatherSelectionKvCache);
#endif

#ifdef GSKVC_BUILD_KV_GATHER
IMPL_OP_OPTILING(KVGather)
    .Tiling(Tiling4KVGather)
    .TilingParse<GatherSelectionKvCacheCompileInfo>(TilingPrepare4GatherSelectionKvCache);
#endif
