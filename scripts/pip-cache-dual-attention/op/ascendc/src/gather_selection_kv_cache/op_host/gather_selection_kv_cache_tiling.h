...
REGISTER_TILING_DATA_CLASS(KVSelect, GatherSelectionKvCacheTilingData)
#endif
#ifdef GSKVC_BUILD_KV_GATHER
REGISTER_TILING_DATA_CLASS(KVGather, GatherSelectionKvCacheTilingData)
#endif
...
enum class GatherSelectionKvCacheTilingOp : uint32_t {
    GATHER_SELECTION_KV_CACHE = 0,
    KV_SELECT = 1,
    KV_GATHER = 2
};
...
public:
    explicit GatherSelectionKvCacheTiling(
        gert::TilingContext* context,
        GatherSelectionKvCacheTilingOp opKind = GatherSelectionKvCacheTilingOp::GATHER_SELECTION_KV_CACHE)
        : context_(context), opKind_(opKind)
    {}
...
private:
    ...
    int32_t FullKRopeIdx() const;
    int32_t FullKvCacheIdx() const;
    int32_t FullKvBlockTableIdx() const;
    int32_t FullKvActualSeqIdx() const;
    int32_t FullQActualSeqIdx() const;
...
private:
    ...
    GatherSelectionKvCacheTilingOp opKind_ = GatherSelectionKvCacheTilingOp::GATHER_SELECTION_KV_CACHE;
