...
constexpr int32_t CURRENT_SEGMENT_HIT_BASE = -10000;
...
public:
    ...
    __aicore__ inline void InitKVSelect(
        GM_ADDR selection_k_rope, GM_ADDR selection_kv_cache, GM_ADDR selection_kv_block_table,
        GM_ADDR selection_kv_block_status, GM_ADDR selection_topk_indices, GM_ADDR full_k_rope, GM_ADDR full_kv_cache,
        GM_ADDR full_kv_block_table, GM_ADDR full_kv_actual_seq, GM_ADDR full_q_actual_seq,
        GM_ADDR hit_sparse_indices, GM_ADDR miss_topk_indices, GM_ADDR miss_insert_indices, GM_ADDR hit_actual_seq,
        GM_ADDR miss_actual_seq, GM_ADDR miss_count, GM_ADDR hit_count, GM_ADDR selection_status_empty)
    {
        ...
        selKvBlockTableUb_ = 0;
        selKvActSeqUb_ = 0;
        selBlockStatUb_ = SH * topkOneSortAlign_ * sizeof(int32_t);
        pipe_->InitBuffer(workBuf_, selBlockStatUb_ + topkSortAlign_ * sizeof(int32_t) * TOPK_NUMS);
        ...
        fullKvActualSeqGm_.SetGlobalBuffer((__gm__ int32_t*)full_kv_actual_seq);
        fullQActualSeqGm_.SetGlobalBuffer((__gm__ int32_t*)full_q_actual_seq);
        hitSparseIndicesGm_.SetGlobalBuffer((__gm__ int32_t*)hit_sparse_indices);
        missTopKIndicesGm_.SetGlobalBuffer((__gm__ int32_t*)miss_topk_indices);
        missInsertIndicesGm_.SetGlobalBuffer((__gm__ int32_t*)miss_insert_indices);
        hitActualSeqGm_.SetGlobalBuffer((__gm__ int32_t*)hit_actual_seq);
        missActualSeqGm_.SetGlobalBuffer((__gm__ int32_t*)miss_actual_seq);
        missCountGm_.SetGlobalBuffer((__gm__ int32_t*)miss_count);
        hitCountGm_.SetGlobalBuffer((__gm__ int32_t*)hit_count);
        selectionStatusEmptyGm_.SetGlobalBuffer((__gm__ int32_t*)selection_status_empty);
    }

    __aicore__ inline void InitKVGather(
        GM_ADDR selection_k_rope, GM_ADDR selection_kv_cache, GM_ADDR selection_kv_block_table,
        ...
        GM_ADDR selection_status_empty, GM_ADDR selection_kv_actual_seq)
    {
        (void)full_q_actual_seq;
        ...
        pipe_->InitBuffer(workBuf_,
                          selKvBlockTableUb_ + selKvActSeqUb_ + selBlockStatUb_ +
                          topkSortAlign_ * sizeof(int32_t));
        ...
        selKvBlockTableGm_.SetGlobalBuffer((__gm__ int32_t*)selection_kv_block_table);
        selKvBlockStatusGm_.SetGlobalBuffer((__gm__ int32_t*)selection_kv_block_status);
        selTopKIndicesGm_.SetGlobalBuffer((__gm__ int32_t*)miss_topk_indices);
        missInsertIndicesGm_.SetGlobalBuffer((__gm__ int32_t*)miss_insert_indices);
        fullKRopeGm_.SetGlobalBuffer((__gm__ T*)full_k_rope);
        fullKvCacheGm_.SetGlobalBuffer((__gm__ T*)full_kv_cache);
        fullKvBlockTableGm_.SetGlobalBuffer((__gm__ int32_t*)full_kv_block_table);
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t bsIdx = 0; bsIdx < bsLoopNum_; bsIdx++) {
            int64_t curBatchSize = 0;
            int64_t curSeq = 0;
            GetBatchSeq(bsIdx, curBatchSize, curSeq);
    ...
    __aicore__ inline void ProcessKVSelect()
    {
        if (blkIdx_ >= tiling_->usedCoreNum) {
            return;
        }

        LocalTensor<int32_t> selBlockStatLocal = workBuf_.Get<int32_t>();
        topkIndicesLocal_ = selBlockStatLocal[selBlockStatUb_ / sizeof(int32_t)];
        insertStatusSameSeqLocal_ = topkIndicesLocal_[topkSortAlign_];
        hitFromSrcSeqLocal_ = insertStatusSameSeqLocal_[topkSortAlign_];
        sortBuf_ = hitFromSrcSeqLocal_[topkSortAlign_];
        ...
        for (int64_t bsIdx = 0; bsIdx < bsLoopNum_; bsIdx++) {
            ...
            if (curFullKvSeqLen <= 0) {
                ClearKVSelectOutput(curBatchSize, curSeq);
                continue;
            }

            bool emptySelectionStatus = IsSelectionStatusEmpty(curBatchSize, curSeq);
            SetWaitFlag<HardEvent::MTE3_MTE2>(HardEvent::MTE3_MTE2);
            CopyInTopKIndices(bsIdx);
            if (!emptySelectionStatus) {
                CopyInBlockStatus(bsIdx, selBlockStatLocal);
            }
            LocalTensor<int32_t> selTopKIdxLocal = selTopKIdxQue_.DeQue<int32_t>();
            SetWaitFlag<HardEvent::MTE2_S>(HardEvent::MTE2_S);
            ...
            if (curFullKvSeqModify > 0) {
                for (int64_t hnIdx = 0; hnIdx < tiling_->headnum; hnIdx++) {
                    LocalTensor<int32_t> tmpSelTopkLocal = selTopKIdxLocal[hnIdx * topkSortAlign_];
                    if (emptySelectionStatus) {
                        ProcessKVSelectTopKEmptyStatus(curBatchSize, curSeq, hnIdx, curFullKvSeqModify,
                            tmpSelTopkLocal);
                    } else {
                        ArithProgression<int32_t>(topkIndicesLocal_, 0, 1, topkSortAlign_);
                        PipeBarrier<PIPE_V>();
                        ProcessKVSelectTopK(curBatchSize, curSeq, hnIdx, curFullKvSeqModify, selBlockStatLocal,
                            tmpSelTopkLocal);
                    }
                }
            } else {
    ...
    __aicore__ inline void ProcessKVGather()
    {
        if (blkIdx_ >= tiling_->usedCoreNum) {
            return;
        }
        ...
        LocalTensor<int32_t> selKvActSeqLocal = selKvBlockTableLocal[selKvBlockTableUb_ / sizeof(int32_t)];
        LocalTensor<int32_t> selBlockStatLocal = selKvActSeqLocal[selKvActSeqUb_ / sizeof(int32_t)];
        LocalTensor<int32_t> missInsertLocal = selBlockStatLocal[selBlockStatUb_ / sizeof(int32_t)];
        ...
        for (int64_t bsIdx = 0; bsIdx < bsLoopNum_; bsIdx++) {
            ...
            if (curFullKvSeqLen <= tiling_->seq) {
                WriteSelectionActualSeq(curBatchSize, curSeq, static_cast<int32_t>(curFullKvSeqLen));
                continue;
            }
            int64_t curFullKvSeqModify = curFullKvSeqLen - offset;

            int64_t row = curBatchSize * rawSeq_ + curSeq;
            int32_t hitActualSeqLen = hitActualSeqGm_.GetValue(row);
            int32_t missActualSeqLen = missActualSeqGm_.GetValue(row);
            int32_t missTopKCount = missCountGm_.GetValue(row);
            bool sequentialInsert = selectionStatusEmptyGm_.GetValue(row) != 0;
            int32_t selActualSeqLen = hitActualSeqLen + missActualSeqLen;
            if (missActualSeqLen <= 0 || missTopKCount <= 0) {
                WriteSelectionActualSeq(curBatchSize, curSeq, selActualSeqLen);
                continue;
            }

            CopyInTopKIndices(bsIdx);
            LocalTensor<int32_t> missTopKLocal = selTopKIdxQue_.DeQue<int32_t>();
            SetWaitFlag<HardEvent::MTE2_S>(HardEvent::MTE2_S);
            ...
            if (!sequentialInsert) {
                CopyInBlockStatus(bsIdx, selBlockStatLocal);
                CopyInMissInsertIndices(bsIdx, missTopKCount, missInsertLocal);
            }
            SetWaitFlag<HardEvent::MTE2_S>(HardEvent::MTE2_S);

            if (sequentialInsert) {
                InitCurrentRowBlockStatus(selBlockStatLocal);
                PipeBarrier<PIPE_V>();
            }
            if (curFullKvSeqModify > 0) {
                for (int64_t hnIdx = 0; hnIdx < tiling_->headnum; hnIdx++) {
                    LocalTensor<int32_t> tmpMissTopKLocal = missTopKLocal[hnIdx * topkSortAlign_];
                    LocalTensor<int32_t> tmpMissInsertLocal = missInsertLocal[hnIdx * topkSortAlign_];
                    ProcessMissTopKEmptyStatus(bsIdx, curBatchSize, curSeq, hnIdx, curFullKvSeqModify,
                        selActualSeqLen, missTopKCount, selBlockStatLocal, tmpMissTopKLocal, tmpMissInsertLocal,
                        selKvBlockTableLocal, selKvActSeqLocal, sequentialInsert);
                }
            }

            selTopKIdxQue_.FreeTensor(missTopKLocal);
            SetWaitFlag<HardEvent::S_MTE3>(HardEvent::S_MTE3);
            CopyOutSelKvBlockTableAndSeqLen(bsIdx, selKvActSeqLocal, selBlockStatLocal);
        }
    }
...
private:
    ...
    __aicore__ inline void GetBatchSeq(int64_t bsIdx, int64_t& curBatchSize, int64_t& curSeq) const
    {
        int64_t rowIdx = blkIdx_ * tiling_->mainCoreBsLoopNum + bsIdx;
        curBatchSize = rowIdx / rawSeq_;
        curSeq = rowIdx % rawSeq_;
    }

    __aicore__ inline bool IsCurrentSegmentHit(int32_t hitFromValue) const
    {
        return hitFromValue <= CURRENT_SEGMENT_HIT_BASE;
    }

    __aicore__ inline int32_t CurrentSegmentSlot(int32_t hitFromValue) const
    {
        return CURRENT_SEGMENT_HIT_BASE - hitFromValue;
    }
    ...
    __aicore__ inline void CopyInMissInsertIndices(
        int64_t bsIdx, int32_t missTopKCount, LocalTensor<int32_t>& missInsertLocal)
    {
        int64_t curBatchSize = 0;
        int64_t curSeq = 0;
        GetBatchSeq(bsIdx, curBatchSize, curSeq);
        if (tiling_->headnum == 1 && missTopKCount < tiling_->topk) {
            int64_t SH = rawSeq_ * tiling_->headnum;
            int32_t copyCount = missTopKCount > 0 ? missTopKCount : 0;
            int32_t copyCountAlign = CeilAlign(copyCount, static_cast<int32_t>(BLOCK_BYTES / sizeof(int32_t)));
            uint8_t padCnt = static_cast<uint8_t>(copyCountAlign - copyCount);

            DataCopyPadExtParams<int32_t> dataCopyPadParams{true, 0, padCnt, -1};
            DataCopyExtParams dataCopyParams{
                static_cast<uint16_t>(1),
                static_cast<uint32_t>(copyCount * sizeof(int32_t)), 0, 0, 0};
            DataCopyPad(missInsertLocal,
                missInsertIndicesGm_[curBatchSize * SH * tiling_->topk + curSeq * tiling_->headnum * tiling_->topk],
                dataCopyParams, dataCopyPadParams);
            return;
        }
    ...
    __aicore__ inline void InitCurrentRowBlockStatus(LocalTensor<int32_t>& selBlockStatLocal)
    {
        AscendC::Duplicate(selBlockStatLocal, -1, tiling_->headnum * topkOneSortAlign_);
    }
        ...
        while (curIdx < tiling_->topk && statIdx < tiling_->topk) {
            ...
            if (curTop == statTop) {
                ...
                if (sameSeq) {
                    ...
                    hitFromSrcSeqLocal.SetValue(curRealIdx, CURRENT_SEGMENT_HIT_BASE - statRealIdx);
    ...
    __aicore__ inline void MarkDirectCurrentSegmentHits(int64_t hnIdx, int32_t maxValidCacheId,
        LocalTensor<int32_t>& selBlockStatLocal, LocalTensor<int32_t>& selTopKIdxLocal, int32_t validTopkNum)
    {
        int64_t statOffset = hnIdx * topkOneSortAlign_;
        for (int64_t topKIdx = 0; topKIdx < validTopkNum; topKIdx++) {
            int32_t topKId = selTopKIdxLocal.GetValue(topKIdx);
            if (topKId < 0) {
                break;
            }
            if (topKId > maxValidCacheId || IsCurrentSegmentHit(hitFromSrcSeqLocal_.GetValue(topKIdx))) {
                continue;
            }
            if (selBlockStatLocal.GetValue(statOffset + topKIdx) == topKId) {
                insertStatusSameSeqLocal_.SetValue(topKIdx, topKIdx);
                hitFromSrcSeqLocal_.SetValue(topKIdx, CURRENT_SEGMENT_HIT_BASE - topKIdx);
            }
        }
    }
        ...
        LocalTensor<int32_t>& selKvBlockTableLocal, LocalTensor<int32_t>& selKvActSeqLocal, int64_t curFullQSeqLen)
    {
        ...
        int32_t maxValidCacheId = static_cast<int32_t>(
            (maxSelectionId - seqIdx - 1) > 0 ? (maxSelectionId - seqIdx - 1) : 0);
        ...
        MarkDirectCurrentSegmentHits(hnIdx, maxValidCacheId, selBlockStatLocal, selTopKIdxLocal, validTopkNum);
        ...
        for (int64_t topKIdx = 0; topKIdx < validTopkNum; topKIdx++) {
            ...
            if (IsCurrentSegmentHit(hitFromSrcSeqLocal_.GetValue(topKIdx))) {
                continue;
            }
            ...
            if (selKvBlockNumIdx < 0 || selKvBlockNumIdx >= tiling_->selKvBlockNum) {
                continue;
            }
    ...
    __aicore__ inline void ClearKVSelectOutput(int64_t curBatchSize, int64_t curSeq)
    {
        LocalTensor<int32_t> hitOutLocal = topkIndicesLocal_;
        LocalTensor<int32_t> missOutLocal = sortBuf_;
        LocalTensor<int32_t> missInsertLocal = sortBuf_[topkSortAlign_];
        AscendC::Duplicate(hitOutLocal, -1, topkSortAlign_);
        AscendC::Duplicate(missOutLocal, -1, topkSortAlign_);
        AscendC::Duplicate(missInsertLocal, -1, topkSortAlign_);
        PipeBarrier<PIPE_V>();
        CopyOutKVSelect(curBatchSize, curSeq, hitOutLocal, missOutLocal, missInsertLocal, 0, 0, 0, 0, 0, true);
    }

    __aicore__ inline void WriteSelectionActualSeq(int64_t curBatchSize, int64_t curSeq, int32_t actualSeqLen)
    {
        int64_t SH = rawSeq_ * tiling_->headnum;
        int64_t statusOffset = curBatchSize * SH * (tiling_->topk + 1) +
                               curSeq * tiling_->headnum * (tiling_->topk + 1);
        int64_t seqOffset = (curBatchSize * rawSeq_ + curSeq) * tiling_->headnum;
        for (int64_t hnIdx = 0; hnIdx < tiling_->headnum; hnIdx++) {
            selKvBlockStatusGm_.SetValue(statusOffset + hnIdx * (tiling_->topk + 1) + tiling_->topk,
                                         actualSeqLen);
            selKvActualSeqGm_.SetValue(seqOffset + hnIdx, actualSeqLen);
        }
    }

    __aicore__ inline void ProcessKVSelectTopK(int64_t curBatchSize, int64_t seqIdx, int64_t hnIdx,
        int64_t curFullKvSeqModify, LocalTensor<int32_t>& selBlockStatLocal,
        LocalTensor<int32_t>& selTopKIdxLocal)
    {
        int32_t maxSelectionId = CeilDiv(curFullKvSeqModify, tiling_->selTopKBlockSize) - 1;
        int32_t maxValidCacheId = (maxSelectionId - seqIdx - 1) > 0 ? (maxSelectionId - seqIdx - 1) : 0;
        int64_t lastGatherBlockSize = curFullKvSeqModify - maxSelectionId * tiling_->selTopKBlockSize;
        int32_t validTopkNum = 0;
        int32_t maxTopkId = -1;
        int32_t maxTopkWritedIdx = -1;
        int32_t maxHitSameSeqIdx = -1;
        ...
        int32_t hitCount = 0;
        int32_t missCount = 0;
        int32_t hitActualSeqLen = 0;
        int32_t missActualSeqLen = 0;
        int32_t totalInsertIdx = 0;
        for (int64_t topKIdx = 0; topKIdx < validTopkNum; topKIdx++) {
            int32_t topKId = selTopKIdxLocal.GetValue(topKIdx);
            if (topKId < 0) {
            ...
            int64_t gatherBlockSize = topKId == maxSelectionId ? lastGatherBlockSize : tiling_->selTopKBlockSize;
            int32_t hitFromValue = hitFromSrcSeqLocal_.GetValue(topKIdx);
            if (IsCurrentSegmentHit(hitFromValue)) {
                hitOutLocal.SetValue(hitCount, CurrentSegmentSlot(hitFromValue));
                hitActualSeqLen += gatherBlockSize;
                hitCount++;
            } else {
                int32_t insertIdx = -1;
                for (int32_t insIdx = totalInsertIdx; insIdx < tiling_->topk; insIdx++) {
                ...
                ASSERT_MSG(insertIdx >= 0, "KVSelect failed to find selection insert slot");

                totalInsertIdx = insertIdx + 1;
                missOutLocal.SetValue(missCount, topKId);
                missInsertLocal.SetValue(missCount, insertIdx);
                missActualSeqLen += gatherBlockSize;
                missCount++;
            }
        }

        CopyOutKVSelect(curBatchSize, seqIdx, hitOutLocal, missOutLocal, missInsertLocal,
                    hitActualSeqLen, missActualSeqLen, hitCount, missCount, 0, true);
    }

    __aicore__ inline void ProcessKVSelectTopKEmptyStatus(int64_t curBatchSize, int64_t seqIdx, int64_t hnIdx,
        int64_t curFullKvSeqModify, LocalTensor<int32_t>& selTopKIdxLocal)
    {
        int32_t maxSelectionId = CeilDiv(curFullKvSeqModify, tiling_->selTopKBlockSize) - 1;
        int64_t lastGatherBlockSize = curFullKvSeqModify - maxSelectionId * tiling_->selTopKBlockSize;

        LocalTensor<int32_t> hitOutLocal = topkIndicesLocal_;
        LocalTensor<int32_t> missOutLocal = insertStatusSameSeqLocal_;
        LocalTensor<int32_t> missInsertLocal = hitFromSrcSeqLocal_;
        AscendC::Duplicate(hitOutLocal, -1, topkSortAlign_);

        if (tiling_->selTopKBlockSize == 1 && curFullKvSeqModify >= tiling_->topk) {
            PipeBarrier<PIPE_V>();
            CopyOutKVSelect(curBatchSize, seqIdx, hitOutLocal, selTopKIdxLocal, missInsertLocal,
                        0, tiling_->topk, 0, tiling_->topk, 1, false);
            return;
        }
        ...
        for (int64_t topKIdx = 0; topKIdx < tiling_->topk; topKIdx++) {
            ...
            missOutLocal.SetValue(missCount, topKId);
            missActualSeqLen += topKId == maxSelectionId ? lastGatherBlockSize : tiling_->selTopKBlockSize;
            missCount++;
        }

        CopyOutKVSelect(curBatchSize, seqIdx, hitOutLocal, missOutLocal, missInsertLocal,
                    0, missActualSeqLen, 0, missCount, 1, false);
    }

    __aicore__ inline bool IsSelectionStatusEmpty(int64_t curBatchSize, int64_t curSeq)
    {
        int64_t SH = rawSeq_ * tiling_->headnum;
        int64_t batchOffset = curBatchSize * SH * (tiling_->topk + 1);
        int64_t seqOffset = curSeq * tiling_->headnum * (tiling_->topk + 1);
        for (int64_t hnIdx = 0; hnIdx < tiling_->headnum; hnIdx++) {
            int64_t actualSeqOffset = batchOffset + seqOffset + hnIdx * (tiling_->topk + 1) + tiling_->topk;
            if (selKvBlockStatusGm_.GetValue(actualSeqOffset) > 0) {
                return false;
            }
        }
        return true;
    ...
    __aicore__ inline void ProcessMissTopKEmptyStatus(int64_t bsIdx, int64_t curBatchSize, int64_t seqIdx,
        int64_t hnIdx, int64_t curFullKvSeqModify, int32_t selActualSeqLen, int32_t missTopKCount,
        LocalTensor<int32_t>& selBlockStatLocal, LocalTensor<int32_t>& missTopKLocal,
        LocalTensor<int32_t>& missInsertLocal, LocalTensor<int32_t>& selKvBlockTableLocal,
        LocalTensor<int32_t>& selKvActSeqLocal, bool sequentialInsert)
    {
        (void)curBatchSize;
        SetSelectionKvActualSeqLen(seqIdx, hnIdx, selActualSeqLen, selKvActSeqLocal);
        selBlockStatLocal.SetValue(hnIdx * topkOneSortAlign_ + tiling_->topk, selActualSeqLen);
        if (selActualSeqLen <= 0 || missTopKCount <= 0) {
            return;
        }

        int32_t maxSelectionId = CeilDiv(curFullKvSeqModify, tiling_->selTopKBlockSize) - 1;
        int64_t lastGatherBlockSize = curFullKvSeqModify - maxSelectionId * tiling_->selTopKBlockSize;
        int64_t selBlkTableOffsetLocal = hnIdx * tiling_->selMaxBlockNum;
        int32_t insertIdx = 0;
        int32_t maxTopkId = -1;
        int32_t maxTopkWritedIdx = -1;

        for (int32_t missIdx = 0; missIdx < missTopKCount; missIdx++) {
            int32_t topKId = missTopKLocal.GetValue(missIdx);
            if (topKId > maxSelectionId) {
                continue;
            }

            int64_t gatherBlockSize = topKId == maxSelectionId ? lastGatherBlockSize : tiling_->selTopKBlockSize;
            int32_t selectedInsertIdx = sequentialInsert ? missIdx : missInsertLocal.GetValue(missIdx);
            if (selectedInsertIdx >= 0) {
                insertIdx = selectedInsertIdx;
            }
            if (insertIdx < 0 || insertIdx >= tiling_->topk) {
                continue;
            }
            ...
            selBlockStatLocal.SetValue(hnIdx * topkOneSortAlign_ + insertIdx, topKId);
            CopyFromFullKv(bsIdx, topKId, gatherBlockSize, selKRopeAddr, selKvCacheAddr);
            if (topKId > maxTopkId) {
                maxTopkId = topKId;
                maxTopkWritedIdx = insertIdx;
        ...
        int32_t maxValidIdx = insertIdx - 1;
        if (tiling_->selTopKBlockSize > 1 && maxTopkWritedIdx >= 0 && maxTopkWritedIdx != maxValidIdx) {
            int32_t maxTopkIdBlockSize = maxTopkId == maxSelectionId ? lastGatherBlockSize : tiling_->selTopKBlockSize;
            SwapMaxTopkLast(selBlkTableOffsetLocal, maxTopkWritedIdx, maxValidIdx, maxTopkIdBlockSize,
                            selKvBlockTableLocal);
            int32_t maxIdx = hnIdx * topkOneSortAlign_ + maxTopkWritedIdx;
            int32_t lastIdx = hnIdx * topkOneSortAlign_ + maxValidIdx;
            int32_t lastTopkId = selBlockStatLocal.GetValue(lastIdx);
            selBlockStatLocal.SetValue(lastIdx, maxTopkId);
        ...
        int64_t selKRopeAddr, int64_t selKvCacheAddr)
    {
        ...
        if (kvBlockNumIdx < 0 || kvBlockNumIdx >= tiling_->fullKvBlockNum) {
            return;
        }
    ...
    __aicore__ inline void CopyOutKVSelect(int64_t curBatchSize, int64_t curSeq, LocalTensor<int32_t>& hitOutLocal,
        LocalTensor<int32_t>& missOutLocal, LocalTensor<int32_t>& missInsertLocal,
        int32_t hitActualSeqLen, int32_t missActualSeqLen, int32_t hitCount, int32_t missCount,
        int32_t selectionStatusEmpty, bool writeMissInsert)
    {
        int64_t row = curBatchSize * rawSeq_ + curSeq;
        DataCopyExtParams dataCopyParams{
            static_cast<uint16_t>(1), static_cast<uint32_t>(tiling_->topk * sizeof(int32_t)), 0, 0, 0};
        SetWaitFlag<HardEvent::S_MTE3>(HardEvent::S_MTE3);
        DataCopyPad(hitSparseIndicesGm_[row * tiling_->topk], hitOutLocal, dataCopyParams);
        DataCopyPad(missTopKIndicesGm_[row * tiling_->topk], missOutLocal, dataCopyParams);
        if (writeMissInsert) {
            DataCopyPad(missInsertIndicesGm_[row * tiling_->topk], missInsertLocal, dataCopyParams);
        }

        SetWaitFlag<HardEvent::MTE3_S>(HardEvent::MTE3_S);
        constexpr int32_t scalarStride = BLOCK_BYTES / sizeof(int32_t);
        missInsertLocal.SetValue(0, hitActualSeqLen);
        missInsertLocal.SetValue(scalarStride, missActualSeqLen);
        missInsertLocal.SetValue(scalarStride * 2, hitCount);
        missInsertLocal.SetValue(scalarStride * 3, missCount);
        missInsertLocal.SetValue(scalarStride * 4, selectionStatusEmpty);

        DataCopyExtParams scalarCopyParams{
            static_cast<uint16_t>(1), static_cast<uint32_t>(sizeof(int32_t)), 0, 0, 0};
        SetWaitFlag<HardEvent::S_MTE3>(HardEvent::S_MTE3);
        DataCopyPad(hitActualSeqGm_[row], missInsertLocal, scalarCopyParams);
        DataCopyPad(missActualSeqGm_[row], missInsertLocal[scalarStride], scalarCopyParams);
        DataCopyPad(hitCountGm_[row], missInsertLocal[scalarStride * 2], scalarCopyParams);
        DataCopyPad(missCountGm_[row], missInsertLocal[scalarStride * 3], scalarCopyParams);
        DataCopyPad(selectionStatusEmptyGm_[row], missInsertLocal[scalarStride * 4], scalarCopyParams);
        SetWaitFlag<HardEvent::MTE3_S>(HardEvent::MTE3_S);
    }
...
private:
    ...
    GlobalTensor<int32_t> missTopKIndicesGm_;
    GlobalTensor<int32_t> missInsertIndicesGm_;
    GlobalTensor<int32_t> hitActualSeqGm_;
    GlobalTensor<int32_t> missActualSeqGm_;
    GlobalTensor<int32_t> missCountGm_;
    GlobalTensor<int32_t> hitCountGm_;
    GlobalTensor<int32_t> selectionStatusEmptyGm_;
