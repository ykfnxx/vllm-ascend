...
/*!
 * \file dmp_sparse_flash_attention_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/dmp_sparse_flash_attention_template_tiling_key.h"
#include "dmp_sparse_flash_attention_tiling.h"
...
struct DmpSparseFlashAttentionCompileInfo {
    int64_t core_num;
};
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OPS_LOG_E("DmpSparseFlashAttention", "datatype %d not support", type);
        return "UNDEFINED";
    }
}
...
ge::graphStatus TilingDmpSparseFlashAttention(gert::TilingContext *context)
{
...
ge::graphStatus TilingPrepareForDmpSparseFlashAttention(gert::TilingParseContext *context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OPS_LOG_E("DmpSparseFlashAttention", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
void SFAInfoParser::GetOptionalInputParaInfo()
{
    ...
    opParamInfo_.softmaxMaxOut.tensor = context_->GetOptionalInputTensor(SOFTMAX_MAX_OUT_INPUT_INDEX);
    opParamInfo_.softmaxMaxOut.desc = context_->GetOptionalInputDesc(SOFTMAX_MAX_OUT_INPUT_INDEX);
    opParamInfo_.softmaxSumOut.tensor = context_->GetOptionalInputTensor(SOFTMAX_SUM_OUT_INPUT_INDEX);
    opParamInfo_.softmaxSumOut.desc = context_->GetOptionalInputDesc(SOFTMAX_SUM_OUT_INPUT_INDEX);
    opParamInfo_.priorSoftmaxMax.tensor = context_->GetOptionalInputTensor(PRIOR_SOFTMAX_MAX_INPUT_INDEX);
    opParamInfo_.priorSoftmaxMax.desc = context_->GetOptionalInputDesc(PRIOR_SOFTMAX_MAX_INPUT_INDEX);
    opParamInfo_.priorSoftmaxSum.tensor = context_->GetOptionalInputTensor(PRIOR_SOFTMAX_SUM_INPUT_INDEX);
    opParamInfo_.priorSoftmaxSum.desc = context_->GetOptionalInputDesc(PRIOR_SOFTMAX_SUM_INPUT_INDEX);
    opParamInfo_.priorAttentionOut.tensor = context_->GetOptionalInputTensor(PRIOR_ATTENTION_OUT_INPUT_INDEX);
    opParamInfo_.priorAttentionOut.desc = context_->GetOptionalInputDesc(PRIOR_ATTENTION_OUT_INPUT_INDEX);
}
...
{
    if (context_ == nullptr) {
        OPS_LOG_E("DmpSparseFlashAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    OPS_LOG_FULL(DLOG_INFO, "DmpSparseFlashAttention", "TilingContext: %s", SFADebugTilingContext(context_).c_str());
...
IMPL_OP_OPTILING(DmpSparseFlashAttention)
    .Tiling(TilingDmpSparseFlashAttention)
    .TilingParse<DmpSparseFlashAttentionCompileInfo>(TilingPrepareForDmpSparseFlashAttention);
} // namespace optiling
