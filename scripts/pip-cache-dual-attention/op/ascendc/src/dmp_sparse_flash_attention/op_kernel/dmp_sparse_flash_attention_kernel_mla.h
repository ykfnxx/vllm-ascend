...
/*!
 * \file dmp_sparse_flash_attention_kernel_mla.h
 * \brief
 */

#ifndef DMP_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H
#define DMP_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H
...
#include "dmp_sparse_flash_attention_service_cube_mla.h"
#include "dmp_sparse_flash_attention_service_vector_mla.h"
...
template <typename SFAT> class DmpSparseFlashAttentionMla {
public:
    ...
    __aicore__ inline DmpSparseFlashAttentionMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                __gm__ uint8_t *softmaxMaxOut, __gm__ uint8_t *softmaxSumOut,
                                __gm__ uint8_t *priorSoftmaxMax, __gm__ uint8_t *priorSoftmaxSum,
                                __gm__ uint8_t *priorAttentionOut, __gm__ uint8_t *attentionOut,
                                __gm__ uint8_t *workspace,
                                const DmpSparseFlashAttentionTilingDataMla *__restrict tiling,
...
private:
    ...
    const DmpSparseFlashAttentionTilingDataMla *__restrict tilingData = nullptr;
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::InitTilingData()
{
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
DmpSparseFlashAttentionMla<SFAT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::InitOutputSingleCore()
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::GetActualSeqLen(uint32_t bIdx, uint32_t s1Idx)
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::GetSparseActualSeqLen(uint32_t bIdx, uint32_t s1Idx,
                                                                            uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline uint32_t DmpSparseFlashAttentionMla<SFAT>::GetActualSeqLenKV(uint32_t bIdx)
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::GetPreNextTokensLeftUp()
{
    if (constInfo.sparseMode == 3) {
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::UpdateInnerLoopCond()
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::UpdateInner(uint32_t &s2End, uint32_t &curS2End,
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::Init(__gm__ uint8_t *query,
                       ...
                       __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    constInfo.daExportLse = (softmaxMaxOut != nullptr);
    constInfo.daPriorLse = (priorSoftmaxMax != nullptr);
    constInfo.daPriorAttentionOut = (priorAttentionOut != nullptr);
    ...
    if ASCEND_IS_AIV {
        ...
        if (constInfo.daExportLse || constInfo.daPriorLse || constInfo.daPriorAttentionOut) {
            vectorService.InitDaStateGlobalTensor(
                softmaxMaxOut, softmaxSumOut, priorSoftmaxMax, priorSoftmaxSum, priorAttentionOut);
        }
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::InitCalcParamsEach()
{
    //计算总的基本块
...
template <typename SFAT>
__aicore__ inline void
DmpSparseFlashAttentionMla<SFAT>::Bmm2DataCopyOut(uint64_t attenOutOffset, LocalTensor<OUT_T> &attenOutUb,
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::CalcParams(uint32_t loop, uint64_t s2Start,
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::ComputeMm1(const RunInfo &info)
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::ComputeMm2(const RunInfo &info)
{
    uint32_t nBufferLoopTimes = (info.actMBaseSize + constInfo.nBufferMBaseSize - 1) / constInfo.nBufferMBaseSize;
    uint32_t nBufferTail = info.actMBaseSize - (nBufferLoopTimes - 1) * constInfo.nBufferMBaseSize;
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::Process()
{
    if (aiCoreIdx < usedCoreNum) {
        if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::GetBN2Idx(uint32_t bN2Idx, uint32_t &bIdx,
                                                                                uint32_t &n2Idx)
{
...
template <typename SFAT> __aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::ProcessBalance()
{
...
template <typename SFAT>
__aicore__ inline void
DmpSparseFlashAttentionMla<SFAT>::PreloadPipeline(uint32_t loop, uint64_t s2Start, uint64_t s2LoopIdx,
                                                               RunInfo extraInfo[SFA_PRELOAD_TASK_CACHE_SIZE], uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
{
...
DmpSparseFlashAttentionMla<SFAT>::GetBalanceActualSeqLengths(GlobalTensor<int32_t> &actualSeqLengths,
                                                                          uint32_t bIdx)
{
    if constexpr (LAYOUT_T == SFA_LAYOUT::TND) {
        if (bIdx > 0) {
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                      uint32_t s1GEndPrev,
                                                                                      uint32_t s2EndPrev)
{
...
template <typename SFAT>
__aicore__ inline void DmpSparseFlashAttentionMla<SFAT>::CalcSinnerTopKBegin(RunInfo &info, uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
...
#endif // DMP_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H