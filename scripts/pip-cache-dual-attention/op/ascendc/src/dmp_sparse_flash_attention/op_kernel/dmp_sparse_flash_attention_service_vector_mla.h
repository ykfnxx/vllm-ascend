...
/*!
 * \file dmp_sparse_flash_attention_service_vector_mla.h
 * \brief
 */
#ifndef DMP_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
#define DMP_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
...
#include "dmp_sparse_flash_attention_common.h"
...
public:
    ...
    __aicore__ inline void InitParams(const struct ConstInfo &constInfo,
                                      const DmpSparseFlashAttentionTilingDataMla *__restrict tilingData);
    ...
    __aicore__ inline void InitDaStateGlobalTensor(__gm__ uint8_t *softmaxMaxOut, __gm__ uint8_t *softmaxSumOut,
                                                   __gm__ uint8_t *priorSoftmaxMax,
                                                   __gm__ uint8_t *priorSoftmaxSum,
                                                   __gm__ uint8_t *priorAttentionOut);
    ...
    __aicore__ inline void ExportDaLseToGm(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                           LocalTensor<T> &softmaxSumUb, LocalTensor<T> &softmaxMaxUb);
    // ================================Vector2==========================================
    ...
    __aicore__ inline void DealBmm2ResBaseBlock(const RunInfo &info, const MSplitInfo &mSplitInfo, uint32_t startRow,
                                                ...
                                                uint32_t actualColumnCount);
    __aicore__ inline void MergeDaPriorOutput(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                               LocalTensor<T> &attentionOutUb, LocalTensor<MM2_OUT_T> &priorScratchUb,
                                               uint32_t startRow,
                                               uint32_t dealRowCount, uint32_t columnCount,
                                               uint32_t actualColumnCount);
...
private:
    ...
    GlobalTensor<T> daSoftmaxMaxGm;
    GlobalTensor<T> daSoftmaxSumGm;
    GlobalTensor<T> daPriorMaxGm;
    GlobalTensor<T> daPriorSumGm;
    GlobalTensor<OUT_T> daPriorAttentionOutGm;
...
template <typename SFAT>
__aicore__ inline void SFAVectorService<SFAT>::InitDaStateGlobalTensor(
    __gm__ uint8_t *softmaxMaxOut, __gm__ uint8_t *softmaxSumOut, __gm__ uint8_t *priorSoftmaxMax,
    __gm__ uint8_t *priorSoftmaxSum, __gm__ uint8_t *priorAttentionOut)
{
    if (softmaxMaxOut != nullptr) {
        daSoftmaxMaxGm.SetGlobalBuffer((__gm__ T *)softmaxMaxOut);
    }
    if (softmaxSumOut != nullptr) {
        daSoftmaxSumGm.SetGlobalBuffer((__gm__ T *)softmaxSumOut);
    }
    if (priorSoftmaxMax != nullptr) {
        daPriorMaxGm.SetGlobalBuffer((__gm__ T *)priorSoftmaxMax);
    }
    if (priorSoftmaxSum != nullptr) {
        daPriorSumGm.SetGlobalBuffer((__gm__ T *)priorSoftmaxSum);
    }
    if (priorAttentionOut != nullptr) {
        daPriorAttentionOutGm.SetGlobalBuffer((__gm__ OUT_T *)priorAttentionOut);
    }
}

template <typename SFAT>
__aicore__ inline void SFAVectorService<SFAT>::ExportDaLseToGm(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                                               LocalTensor<T> &softmaxSumUb, LocalTensor<T> &softmaxMaxUb)
{
    if (mSplitInfo.vecDealM == 0 || !constInfo.daExportLse) {
        return;
    }
    uint64_t headStride = constInfo.qHeadNum;
    if (headStride == 0) {
        return;
    }
    // User LSE buffer is dense [T, N] float32. UB softmax rows are per-core local [0, vecDealM);
    // GM head index adds vecStartM for the second vector sub-block.
    uint64_t tokenIndex =
        info.tndBIdxOffsetForQ / (headStride * constInfo.headDim) + info.gS1Idx / constInfo.gSize;
    uint64_t gmOffset =
        tokenIndex * headStride + mSplitInfo.nBufferStartM / 2 + mSplitInfo.vecStartM;
    uint64_t baseOffset = mSplitInfo.nBufferStartM / 2;
    uint32_t calCount = mSplitInfo.vecDealM;
    uint32_t calCountAlign = SFAAlign(calCount, FP32_BLOCK_ELEMENT_NUM);

    WaitFlag<AscendC::HardEvent::MTE3_V>(SYNC_OUTPUT_BUF2_FLAG);
    DataCopy(daSoftmaxSumGm[gmOffset], softmaxSumUb[baseOffset], calCountAlign);
    SetFlag<AscendC::HardEvent::MTE3_V>(SYNC_OUTPUT_BUF2_FLAG);

    WaitFlag<AscendC::HardEvent::MTE3_V>(SYNC_OUTPUT_BUF2_FLAG);
    DataCopy(daSoftmaxMaxGm[gmOffset], softmaxMaxUb[baseOffset], calCountAlign);
    SetFlag<AscendC::HardEvent::MTE3_V>(SYNC_OUTPUT_BUF2_FLAG);
}
    ...
    uint32_t startRow, uint32_t dealRowCount, uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    // With prior_attention_out, keep this SFA call miss-only; Vec2 merges O0 with the miss output.
    if (info.isFirstSInnerLoop && constInfo.daPriorLse && !constInfo.daPriorAttentionOut) {
        inMaxTensor = softmaxMaxUb[softmaxOutOffset];
        inSumTensor = softmaxSumUb[softmaxOutOffset];
        uint64_t headStride = constInfo.qHeadNum;
        uint64_t tokenIndex =
            info.tndBIdxOffsetForQ / (headStride * constInfo.headDim) + info.gS1Idx / constInfo.gSize;
        uint64_t gmOffset =
            tokenIndex * headStride + mSplitInfo.nBufferStartM / 2 + mSplitInfo.vecStartM + startRow;
        uint32_t dealRowCountAlign = SFAAlign(dealRowCount, FP32_BLOCK_ELEMENT_NUM);
        DataCopy(inMaxTensor, daPriorMaxGm[gmOffset], dealRowCountAlign);
        pipe_barrier(PIPE_V);
        DataCopy(inSumTensor, daPriorSumGm[gmOffset], dealRowCountAlign);
        pipe_barrier(PIPE_V);
    ...
    uint32_t startRow, uint32_t dealRowCount, uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    if (info.isFirstSInnerLoop && constInfo.daPriorLse && !constInfo.daPriorAttentionOut) {
        ...
        uint64_t gmOffset =
            tokenIndex * headStride + mSplitInfo.nBufferStartM / 2 + mSplitInfo.vecStartM + startRow;
        uint32_t dealRowCountAlign = SFAAlign(dealRowCount, FP32_BLOCK_ELEMENT_NUM);
        LocalTensor<T> priorMaxUb = nTmp[2 * SOFTMAX_TMP_BUFFER_OFFSET / sizeof(T)];
        DataCopy(priorMaxUb, daPriorMaxGm[gmOffset], dealRowCountAlign);
        pipe_barrier(PIPE_V);
        Muls(priorMaxUb, priorMaxUb, ((T)(-1.0)) * RECIP_OF_LN2, calCount);
        pipe_barrier(PIPE_V);
        Cast(priorMaxUb, priorMaxUb, RoundMode::CAST_ROUND, calCount);
        pipe_barrier(PIPE_V);
        Sub(nUpdateTmp, nTmp, priorMaxUb, calCount);
    } else if (info.isFirstSInnerLoop) {
...
__aicore__ inline void SFAVectorService<SFAT>::ProcessVec1L(const RunInfo &info)
{
    ...
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
        ...
        // move lse for flash decode / dual-attention export
        if (info.s2Idx == info.curSInnerLoopTimes - 1) {
            ...
            if (constInfo.daExportLse) {
                ExportDaLseToGm(info, mSplitInfo, sumTensor, maxTensor);
            }
        }
    }
}
...
template <typename SFAT>
__aicore__ inline void
SFAVectorService<SFAT>::MergeDaPriorOutput(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                            LocalTensor<T> &attentionOutUb,
                                            LocalTensor<MM2_OUT_T> &priorScratchUb,
                                            uint32_t startRow, uint32_t dealRowCount,
                                            uint32_t columnCount, uint32_t actualColumnCount)
{
    if (!constInfo.daPriorAttentionOut || !constInfo.daPriorLse || dealRowCount == 0) {
        return;
    }
    ...
    LocalTensor<T> tmpVec = v0ValidSizeBuff.Get<T>();
    LocalTensor<T> priorMaxUb = tmpVec[0];
    LocalTensor<T> priorSumUb = tmpVec[128];
    LocalTensor<T> mergeScratchUb = tmpVec[256];
    LocalTensor<T> priorWeightUb = tmpVec[384];
    LocalTensor<T> missWeightUb = tmpVec[512];
    LocalTensor<T> rowWeightBrcbUb = tmpVec[640];

    DataCopy(priorMaxUb, daPriorMaxGm[lseGmOffset], dealRowCountAlign);
    pipe_barrier(PIPE_V);
    DataCopy(priorSumUb, daPriorSumGm[lseGmOffset], dealRowCountAlign);
    pipe_barrier(PIPE_V);

    Max(mergeScratchUb, priorMaxUb, softmaxMaxUb[idx * SOFTMAX_TMP_BUFFER_OFFSET / sizeof(T) + baseOffset],
        dealRowCount);
    pipe_barrier(PIPE_V);
    Sub(priorWeightUb, priorMaxUb, mergeScratchUb, dealRowCount);
    Sub(missWeightUb, softmaxMaxUb[idx * SOFTMAX_TMP_BUFFER_OFFSET / sizeof(T) + baseOffset], mergeScratchUb,
        dealRowCount);
    pipe_barrier(PIPE_V);
    Exp(priorWeightUb, priorWeightUb, dealRowCount);
    Exp(missWeightUb, missWeightUb, dealRowCount);
    pipe_barrier(PIPE_V);
    Mul(priorWeightUb, priorWeightUb, priorSumUb, dealRowCount);
    Mul(missWeightUb, missWeightUb, softmaxSumUb[idx * SOFTMAX_TMP_BUFFER_OFFSET / sizeof(T) + baseOffset],
        dealRowCount);
    pipe_barrier(PIPE_V);
    Add(mergeScratchUb, priorWeightUb, missWeightUb, dealRowCount);
    pipe_barrier(PIPE_V);
    Div(priorWeightUb, priorWeightUb, mergeScratchUb, dealRowCount);
    Div(missWeightUb, missWeightUb, mergeScratchUb, dealRowCount);
    pipe_barrier(PIPE_V);
    Brcb(rowWeightBrcbUb, missWeightUb, (dealRowCount + 7) / 8, {1, 8});
    pipe_barrier(PIPE_V);
    RowMuls(attentionOutUb, attentionOutUb, rowWeightBrcbUb, dealRowCount, columnCount, actualColumnCount);
    pipe_barrier(PIPE_V);
    Brcb(rowWeightBrcbUb, priorWeightUb, (dealRowCount + 7) / 8, {1, 8});
    pipe_barrier(PIPE_V);

    uint32_t vec2ComputeSize = dealRowCount * columnCount;
    uint32_t mStart = mSplitInfo.nBufferStartM + mSplitInfo.vecStartM + startRow;
    uint64_t priorOutOffset = info.attenOutOffset + mStart * actualColumnCount;
    LocalTensor<OUT_T> priorOutUb = inputBuff2.Get<OUT_T>();
    WaitFlag<AscendC::HardEvent::V_MTE2>(SYNC_INPUT_BUF2_FLAG);
    DataCopyExtParams dataCopyParams;
    dataCopyParams.blockCount = dealRowCount;
    dataCopyParams.blockLen = actualColumnCount * sizeof(OUT_T);
    dataCopyParams.srcStride = 0;
    dataCopyParams.dstStride = 0;
    DataCopyPadExtParams<OUT_T> padParams;
    DataCopyPad(priorOutUb, daPriorAttentionOutGm[priorOutOffset], dataCopyParams, padParams);
    SetFlag<AscendC::HardEvent::MTE2_V>(SYNC_INPUT_BUF2_FLAG);
    WaitFlag<AscendC::HardEvent::MTE2_V>(SYNC_INPUT_BUF2_FLAG);

    LocalTensor<T> priorOutFloatUb = priorScratchUb;
    Cast(priorOutFloatUb, priorOutUb, RoundMode::CAST_NONE, vec2ComputeSize);
    pipe_barrier(PIPE_V);
    RowMuls(priorOutFloatUb, priorOutFloatUb, rowWeightBrcbUb, dealRowCount, columnCount, actualColumnCount);
    pipe_barrier(PIPE_V);
    Add(attentionOutUb, attentionOutUb, priorOutFloatUb, vec2ComputeSize);
    pipe_barrier(PIPE_V);
    SetFlag<AscendC::HardEvent::V_MTE2>(SYNC_INPUT_BUF2_FLAG);
}
                                                           ...
                                                           uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    pipe_barrier(PIPE_V);
    MergeDaPriorOutput(info, mSplitInfo, bmm2ResUb, tmpBmm2ResUb, startRow, dealRowCount, columnCount,
                       actualColumnCount);
    SetFlag<AscendC::HardEvent::V_MTE2>(SYNC_INPUT_BUF1_FLAG + pingpongFlag);
...
#endif // DMP_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
