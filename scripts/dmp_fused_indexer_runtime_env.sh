...
if [[ "$VLLM_ASCEND_ENABLE_DMP_FUSED_INDEXER_KV_SELECT" == "1" ]]; then
    ...
    export PYTHONPATH="$_dmp_fused_python:$_dmp_fused_lookup_python:$_dmp_fused_dual_python${PYTHONPATH:+:$PYTHONPATH}"
    ...
    for _dmp_fused_entry in "${_dmp_fused_opp_entries[@]}"; do
        ...
        case "${_dmp_fused_entry%/}" in
            "${_dmp_fused_vendor%/}"|"${_dmp_fused_lookup_vendor%/}"|"${_dmp_fused_dual_vendor%/}")
                continue
                ;;
        esac
...
unset _dmp_fused_nounset_was_on _dmp_fused_script_dir _dmp_fused_vendor \
    _dmp_fused_python _dmp_fused_lookup_vendor _dmp_fused_lookup_python \
    _dmp_fused_dual_vendor _dmp_fused_dual_python _dmp_fused_filtered_opp \
    _dmp_fused_opp_entries _dmp_fused_entry
