...
BOTH_KV_PORT="30200"
PREFILL_DP_RPC_PORT="19100"
DECODE_DP_RPC_PORT="19200"
...
Core options:
  ...
  --prefill-device IDS        P/D Prefill physical NPU(s), comma-separated.
                              Default: 0
  --decode-device IDS         P/D Decode physical NPU(s), comma-separated.
                              Default: 1
  --both-device IDS           kv_both physical NPU(s), comma-separated.
                              Default: 0
  --data-parallel-size N      Data-parallel size per engine. Default: 1.
                              Each engine's device list must have N entries.
  --enable-expert-parallel    Enable expert parallelism for the MoE experts.
                              Tensor parallelism stays at 1, so EP runs across
...
Port options:
  ...
  --prefill-dp-rpc-port PORT  Default: 19100
  --decode-dp-rpc-port PORT   Default: 19200
  --both-dp-rpc-port PORT     Default: 19300
...
while (($# > 0)); do
    case "$1" in
        ...
        --prefill-dp-rpc-port)
            require_value "$@"
            PREFILL_DP_RPC_PORT="$2"
            shift 2
            ;;
        --decode-dp-rpc-port)
            require_value "$@"
            DECODE_DP_RPC_PORT="$2"
            shift 2
            ;;
        --both-dp-rpc-port)
            require_value "$@"
...
device_count() {
    local devices="$1"
    local -a entries
    IFS=',' read -ra entries <<<"$devices"
    echo "${#entries[@]}"
}

device_lists_overlap() {
    local -a left right
    local l r
    IFS=',' read -ra left <<<"$1"
    IFS=',' read -ra right <<<"$2"
...
if [[ "$SCENARIO" == "pd" ]]; then
    if [[ "$(device_count "$PREFILL_DEVICE")" != "$DATA_PARALLEL_SIZE" ]] \
        || [[ "$(device_count "$DECODE_DEVICE")" != "$DATA_PARALLEL_SIZE" ]]; then
        echo "Prefill and Decode device lists must each contain data-parallel-size entries." >&2
        exit 2
    fi
    if device_lists_overlap "$PREFILL_DEVICE" "$DECODE_DEVICE"; then
        echo "Prefill and Decode must use disjoint physical NPU IDs." >&2
        exit 2
    fi
elif [[ "$(device_count "$BOTH_DEVICE")" != "$DATA_PARALLEL_SIZE" ]]; then
    echo "The kv_both device list must contain data-parallel-size entries." >&2
    exit 2
fi
if [[ "$ENABLE_EXPERT_PARALLEL" == "1" && "$DATA_PARALLEL_SIZE" == "1" ]]; then
    echo "enable-expert-parallel with tensor-parallel-size 1 requires --data-parallel-size > 1." >&2
    exit 2
fi
...
extra = {
    "prefill": {"dp_size": int(dp_size), "tp_size": 1},
    "decode": {"dp_size": int(dp_size), "tp_size": 1},
}
