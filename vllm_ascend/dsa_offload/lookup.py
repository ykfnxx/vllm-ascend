...
def pack_decode_metadata(batch: DSAOffloadBatch) -> PackedDecodeMetadata:
    ...
    query_positions = batch.query_positions.index_select(0, token_indices)
...
def make_lookup_plan(
    *,
    ...
    mapped_indices, miss_out = lookup_update_batch(
        state,
        batch.request_rows,
        ...
        tail_base=batch.layout.tail_base,
        fallback_slot=batch.layout.fallback_slot,
        staging_base=batch.layout.staging_base,
        decode_mode=int(batch.is_mtp),
    )
    query_indices = semantic_topk.reshape(semantic_topk.shape[0], -1)
    lookup_slots = mapped_indices.reshape(mapped_indices.shape[0], -1)
    dense_miss_mask = miss_out.reshape(miss_out.shape[0], -1)
    query_lengths = (
        batch.query_start_loc[1:] - batch.query_start_loc[:-1]
    ).to(torch.int64)
    query_request_rows = torch.repeat_interleave(
        batch.request_rows,
        query_lengths,
    ...
    if graph_mode:
        ...
        miss_batch_indices = query_indices.new_empty((0,), dtype=torch.int32)
    else:
        active_misses = dense_miss_mask.bool()
        query_batch_indices = torch.repeat_interleave(
            torch.arange(
                batch.request_rows.shape[0],
                dtype=torch.int32,
                device=batch.request_rows.device,
            ),
            query_lengths,
            output_size=query_indices.shape[0],
        )
        expanded_rows = query_request_rows.unsqueeze(1).expand_as(query_indices)
        expanded_batch_indices = query_batch_indices.unsqueeze(1).expand_as(
            query_indices
        )
        miss_positions = query_indices[active_misses].to(torch.int64)
        ...
        miss_slots = lookup_slots[active_misses].to(torch.int64)
    ...
    return LookupPlan(
        ...
        lookup_slots=lookup_slots if use_dense_gather else None,
        dense_miss_mask=(
            dense_miss_mask if use_dense_gather else None
        ),
...
def make_prefetch_lookup_plan(
    *,
    ...
    packed_positions = batch.query_positions if graph_mode else packed.query_positions
    query_lengths = (
    ...
    use_turbo = batch.is_mtp and batch.enable_turbo_prefetch_lookup
    if use_turbo:
        ...
        lookup_mask = (
            (query_indices >= 0)
            & (query_indices < INDEX_CAPACITY)
            & (query_indices < tail_starts.unsqueeze(1))
        ).to(torch.int32).contiguous()
        slot_out, miss_out = turbo_prefetch_lookup_update_batch(
            state,
            request_rows,
            query_start_loc,
            query_indices,
        ...
        mapped_slot_out = batch.layout.lookup_offsets(slot_out)
        active = miss_out.bool() & lookup_mask.bool()
    else:
        mapped_indices, miss_mask = lookup_update_batch(
            state,
            request_rows,
            query_start_loc,
            packed_positions,
        ...
        mapped_slot_out = mapped_indices.reshape(query_indices.shape)
        active = miss_mask.reshape(query_indices.shape).bool()
    if graph_mode or use_dense_gather:
        ...
        miss_slots = mapped_slot_out[active].to(torch.int64)
        miss_destination_slots = (
            batch.layout.hot_block_base
            + miss_rows * batch.layout.hot_blocks_per_row
        ) * batch.layout.block_size + miss_slots
    ...
    return PrefetchLookupPlan(
        query_indices=query_indices if use_dense_gather else None,
        lookup_slots=(
            mapped_slot_out if use_dense_gather else None
        ),
        dense_miss_mask=(
