...
constexpr int32_t DSA_ROW_MODE_PAD = 0;
constexpr int32_t DSA_ROW_MODE_DENSE = 1;
constexpr int32_t DSA_ROW_MODE_SPARSE = 2;
...
public:
    ...
    __aicore__ inline void InitRowMode(
        GM_ADDR selection_k_rope, GM_ADDR selection_kv_cache, GM_ADDR selection_kv_block_table,
        GM_ADDR selection_kv_block_status, GM_ADDR req_pool_entries, GM_ADDR selection_topk_indices,
        GM_ADDR full_k_rope, GM_ADDR full_kv_cache, GM_ADDR full_kv_block_table, GM_ADDR full_kv_actual_seq,
        GM_ADDR row_modes, GM_ADDR budget_lengths, GM_ADDR tail_valid_token_counts,
        GM_ADDR resident_tail_starts, GM_ADDR query_position_rows, GM_ADDR attention_indices_out)
    {
        Init(selection_k_rope, selection_kv_cache, selection_kv_block_table, selection_kv_block_status,
             req_pool_entries, selection_topk_indices, full_k_rope, full_kv_cache, full_kv_block_table,
             full_kv_actual_seq);
        rowMode_ = true;
        rowModesGm_.SetGlobalBuffer((__gm__ int32_t*)row_modes);
        budgetLengthsGm_.SetGlobalBuffer((__gm__ int32_t*)budget_lengths);
        tailValidTokenCountsGm_.SetGlobalBuffer((__gm__ int32_t*)tail_valid_token_counts);
        residentTailStartsGm_.SetGlobalBuffer((__gm__ int32_t*)resident_tail_starts);
        queryPositionRowsGm_.SetGlobalBuffer((__gm__ int32_t*)query_position_rows);
        attentionIndicesOutGm_.SetGlobalBuffer((__gm__ int32_t*)attention_indices_out);
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t bsIdx = 0; bsIdx < bsLoopNum_; bsIdx++) {
            ...
            if (rowMode_) {
                int32_t rowMode = rowModesGm_.GetValue(curBatchSize);
                if (rowMode == DSA_ROW_MODE_PAD) {
                    WriteInvalidAttention(curBatchSize);
                    continue;
                }
                if (rowMode == DSA_ROW_MODE_DENSE) {
                    WriteDenseAttention(curBatchSize, curSeq);
                    continue;
                }
                if (rowMode != DSA_ROW_MODE_SPARSE) {
                    WriteInvalidAttention(curBatchSize);
                    continue;
                }
            }
            ...
            if (curFullKvSeqLen <= tiling_->topk) {
                // In the current vLLM-Ascend DSA contract, rows whose visible
                // candidate window fits within topk should be represented as
                // DSA_ROW_MODE_DENSE by Python.  A SPARSE row reaching this
                // branch is kept valid for defensive safety and writes the
                // already resident slot indices without touching DRAM/HBM
                // status, but new sparse-stage policies should not rely on
                // this as the normal materialization path.
                if (rowMode_) {
                    WriteSparseAttention(curBatchSize);
                }
                continue;
            }
...
private:
    ...
    __aicore__ inline int32_t ClampValue(int32_t value, int32_t low, int32_t high)
    {
        int32_t clamped = value < low ? low : value;
        return clamped > high ? high : clamped;
    }

    __aicore__ inline void WriteInvalidAttention(int64_t batchRow)
    {
        int64_t width = tiling_->attentionIndicesWidth;
        int64_t base = batchRow * width;
        for (int64_t idx = 0; idx < width; ++idx) {
            attentionIndicesOutGm_.SetValue(base + idx, -1);
        }
    }

    __aicore__ inline void WriteDenseAttention(int64_t batchRow, int64_t seqIdx)
    {
        // Dense rows do not use the sparse resident-slot table. The values
        // written here are the native dense positions selected by lightning
        // indexer for the dense/full-cache SFA path, not DSA resident slots.
        int64_t width = tiling_->attentionIndicesWidth;
        int64_t base = batchRow * width;
        int64_t topkBase = batchRow * rawSeq_ * tiling_->headnum * tiling_->topk +
            seqIdx * tiling_->headnum * tiling_->topk;
        for (int64_t idx = 0; idx < width; ++idx) {
            int32_t value = idx < tiling_->topk ? selTopKIndicesGm_.GetValue(topkBase + idx) : -1;
            attentionIndicesOutGm_.SetValue(base + idx, value);
        }
    }

    __aicore__ inline void WriteSparseAttention(int64_t batchRow)
    {
        // Sparse attention indices are resident logical slot ids consumed by
        // SFA, not original sequence token ids.  The token->slot mapping lives
        // in selection_kv_block_status and is refreshed in place below.  For
        // example, if status changes from [1,2,3,4,5] to [1,8,9,4,5], SFA
        // still receives [0,1,2,3,4] so it reads resident slots 0..4.
        int32_t width = static_cast<int32_t>(tiling_->attentionIndicesWidth);
        int64_t base = batchRow * width;
        int32_t budget = ClampValue(budgetLengthsGm_.GetValue(batchRow), 0, width);
        int32_t tailCount = ClampValue(tailValidTokenCountsGm_.GetValue(batchRow), 0, width);
        if (budget + tailCount > width) {
            tailCount = width - budget;
        }
        int32_t tailStart = residentTailStartsGm_.GetValue(batchRow);
        int32_t queryPos = -1;
        if (tiling_->queryPositionWidth > 0) {
            queryPos = queryPositionRowsGm_.GetValue(batchRow * tiling_->queryPositionWidth);
        }
        int32_t tailEnd = tailStart + tailCount;
        bool queryExtraValid = queryPos >= 0 && queryPos >= budget &&
            !(queryPos >= tailStart && queryPos < tailEnd) &&
            (budget + tailCount) < width;
        int32_t queryExtraCol = budget + tailCount;

        for (int32_t idx = 0; idx < width; ++idx) {
            int32_t value = -1;
            if (idx < budget) {
                // Important: idx is the resident sparse slot, not the original
                // token position represented by that slot.
                value = idx;
            } else {
                int32_t tailOffset = idx - budget;
                if (tailOffset >= 0 && tailOffset < tailCount) {
                    // Tail tokens remain dense in the resident MLA cache, so
                    // their SFA indices are resident tail slot ids as well.
                    value = tailStart + tailOffset;
                }
            }
            if (queryExtraValid && idx == queryExtraCol) {
                value = queryPos;
            }
            attentionIndicesOutGm_.SetValue(base + idx, value);
        }
        ...
        LocalTensor<int32_t>& selKvBlockTableLocal, LocalTensor<int32_t>& selKvActSeqLocal, int64_t curFullQSeqLen)
    {
        ...
        for (int64_t topKIdx = 0; topKIdx < validTopkNum; topKIdx++) {
            ...
            if (hitFromSrcSeqLocal_.GetValue(topKIdx) == CUR_SEG_HIT_FLAG) {
                // The requested token/segment is already resident in this row.
                // Keep it in its current slot to avoid a pointless HBM move;
                // selection_kv_block_status already records the mapping.
                continue;
            }
            ...
            // 原址刷新 selection_kv_block_status：status[resident_slot] =
            // original token/segment id.  This table, rather than
            // attention_indices_out, is the persistent mapping used by later
            // decode steps to decide whether a topK item is already resident.
...
private:
    ...
    GlobalTensor<int32_t> budgetLengthsGm_;
    GlobalTensor<int32_t> tailValidTokenCountsGm_;
    GlobalTensor<int32_t> residentTailStartsGm_;
    GlobalTensor<int32_t> queryPositionRowsGm_;
    GlobalTensor<int32_t> attentionIndicesOutGm_;
    bool rowMode_ = false;
