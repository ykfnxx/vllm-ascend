...
constexpr int32_t ROW_MODES_IDX = 10;
constexpr int32_t BUDGET_LENGTHS_IDX = 11;
constexpr int32_t TAIL_VALID_TOKEN_COUNTS_IDX = 12;
constexpr int32_t RESIDENT_TAIL_STARTS_IDX = 13;
constexpr int32_t QUERY_POSITION_ROWS_IDX = 14;
constexpr int32_t ATTENTION_INDICES_OUT_IDX = 15;
...
ge::graphStatus GatherSelectionKvCacheTiling::GetShapeAttrsInfo()
{
    ...
    tilingData_.set_attentionIndicesWidth(0);
    tilingData_.set_queryPositionWidth(0);
    if (rowMode_ && GetRowModeInfo() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus GatherSelectionKvCacheTiling::GetRowModeInfo()
{
    auto checkVectorInput = [this](int32_t inputIdx, const char* name) -> ge::graphStatus {
        auto input = context_->GetInputShape(inputIdx);
        OPS_ERR_IF(input == nullptr, OPS_LOG_E(context_->GetNodeName(), "get %s nullptr.", name),
            return ge::GRAPH_FAILED);
        gert::Shape shape = input->GetStorageShape();
        OPS_ERR_IF(
            (shape.GetDimNum() != CONST1),
            OPS_LOG_E(context_->GetNodeName(), "%s dim:%lu should be 1.", name, shape.GetDimNum()),
            return ge::GRAPH_FAILED);
        OPS_ERR_IF(
            (shape.GetDim(0) != batchSize_),
            OPS_LOG_E(context_->GetNodeName(), "%s dim0:%ld should be batchSize:%ld.",
                name, shape.GetDim(0), batchSize_),
            return ge::GRAPH_FAILED);
        return ge::GRAPH_SUCCESS;
    };

    if (checkVectorInput(ROW_MODES_IDX, "row_modes") != ge::GRAPH_SUCCESS ||
        checkVectorInput(BUDGET_LENGTHS_IDX, "budget_lengths") != ge::GRAPH_SUCCESS ||
        checkVectorInput(TAIL_VALID_TOKEN_COUNTS_IDX, "tail_valid_token_counts") != ge::GRAPH_SUCCESS ||
        checkVectorInput(RESIDENT_TAIL_STARTS_IDX, "resident_tail_starts") != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    auto queryPositionRows = context_->GetInputShape(QUERY_POSITION_ROWS_IDX);
    OPS_ERR_IF(queryPositionRows == nullptr,
        OPS_LOG_E(context_->GetNodeName(), "get query_position_rows nullptr."),
        return ge::GRAPH_FAILED);
    gert::Shape queryPositionRowsShape = queryPositionRows->GetStorageShape();
    OPS_ERR_IF(
        (queryPositionRowsShape.GetDimNum() != CONST2),
        OPS_LOG_E(context_->GetNodeName(), "query_position_rows dim:%lu should be 2.",
            queryPositionRowsShape.GetDimNum()),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        (queryPositionRowsShape.GetDim(0) != batchSize_),
        OPS_LOG_E(context_->GetNodeName(), "query_position_rows dim0:%ld should be batchSize:%ld.",
            queryPositionRowsShape.GetDim(0), batchSize_),
        return ge::GRAPH_FAILED);
    tilingData_.set_queryPositionWidth(queryPositionRowsShape.GetDim(1));

    auto attentionIndicesOut = context_->GetInputShape(ATTENTION_INDICES_OUT_IDX);
    OPS_ERR_IF(attentionIndicesOut == nullptr,
        OPS_LOG_E(context_->GetNodeName(), "get attention_indices_out nullptr."),
        return ge::GRAPH_FAILED);
    gert::Shape attentionIndicesOutShape = attentionIndicesOut->GetStorageShape();
    size_t dimsN = attentionIndicesOutShape.GetDimNum();
    OPS_ERR_IF(
        (dimsN != CONST2 && dimsN != CONST3),
        OPS_LOG_E(context_->GetNodeName(), "attention_indices_out dim:%lu should be 2 or 3.", dimsN),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        (attentionIndicesOutShape.GetDim(0) != batchSize_),
        OPS_LOG_E(context_->GetNodeName(), "attention_indices_out dim0:%ld should be batchSize:%ld.",
            attentionIndicesOutShape.GetDim(0), batchSize_),
        return ge::GRAPH_FAILED);
    if (dimsN == CONST3) {
        OPS_ERR_IF(
            (attentionIndicesOutShape.GetDim(1) != CONST1),
            OPS_LOG_E(context_->GetNodeName(), "attention_indices_out dim1:%ld should be 1.",
                attentionIndicesOutShape.GetDim(1)),
            return ge::GRAPH_FAILED);
        tilingData_.set_attentionIndicesWidth(attentionIndicesOutShape.GetDim(2));
    } else {
        tilingData_.set_attentionIndicesWidth(attentionIndicesOutShape.GetDim(1));
    }
    OPS_ERR_IF(
        (tilingData_.get_attentionIndicesWidth() <= 0),
        OPS_LOG_E(context_->GetNodeName(), "attention_indices_out width must be positive."),
        return ge::GRAPH_FAILED);

    auto attentionIndicesDesc = context_->GetInputDesc(ATTENTION_INDICES_OUT_IDX);
    OPS_ERR_IF(attentionIndicesDesc == nullptr,
        OPS_LOG_E(context_->GetNodeName(), "get attention_indices_out desc nullptr."),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        (attentionIndicesDesc->GetDataType() != ge::DT_INT32),
        OPS_LOG_E(context_->GetNodeName(), "attention_indices_out dtype should be int32."),
        return ge::GRAPH_FAILED);
...
ge::graphStatus GatherSelectionKvCacheTiling::GetInputDtypeInfo()
{
    ...
    if (rowMode_) {
        const int32_t rowModeInputIdxs[] = {
            ROW_MODES_IDX,
            BUDGET_LENGTHS_IDX,
            TAIL_VALID_TOKEN_COUNTS_IDX,
            RESIDENT_TAIL_STARTS_IDX,
            QUERY_POSITION_ROWS_IDX,
            ATTENTION_INDICES_OUT_IDX,
        };
        const char* rowModeInputNames[] = {
            "row_modes",
            "budget_lengths",
        ...
        for (size_t idx = 0; idx < sizeof(rowModeInputIdxs) / sizeof(rowModeInputIdxs[0]); ++idx) {
            auto desc = context_->GetInputDesc(rowModeInputIdxs[idx]);
            OPS_ERR_IF(desc == nullptr,
                OPS_LOG_E(context_->GetNodeName(), "get %s desc nullptr.", rowModeInputNames[idx]),
                return ge::GRAPH_FAILED);
            OPS_ERR_IF(
                (desc->GetDataType() != ge::DT_INT32),
                OPS_LOG_E(context_->GetNodeName(), "%s dtype should be int32.", rowModeInputNames[idx]),
                return ge::GRAPH_FAILED);
        }
    }
...
ge::graphStatus GatherSelectionKvCacheTiling::DoOpTiling()
{
    ...
    tilingKey_ = 0;
    if (rowMode_) {
        tilingKey_ = 3; // 3: row-mode full decode batch, vec implementation
...
void GatherSelectionKvCacheTiling::PrintTilingDatas()
{
    OPS_LOG_I(
        ...
        "tilingData is coreNum:%ld ubSize:%ld usedCoreNum:%ld mainCoreBsLoopNum:%ld tailCoreBsLoopNum:%ld, \
          ...
          layOut:%ld attentionIndicesWidth:%ld queryPositionWidth:%ld rowMode:%d",
        ...
        tilingData_.get_layOut(), tilingData_.get_attentionIndicesWidth(),
        tilingData_.get_queryPositionWidth(), rowMode_ ? 1 : 0);
}
