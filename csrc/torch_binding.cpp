...
#include "gather_selection_kv_cache/gather_selection_kv_cache_torch_adpt.h"
...
gather_selection_kv_cache_torch_op(
...
{
    TORCH_CHECK(selection_k_rope.device().is_privateuseone(),
                "selection_k_rope must be on NPU.");
    TORCH_CHECK(selection_kv_cache.device().is_privateuseone(),
                "selection_kv_cache must be on NPU.");
    TORCH_CHECK(selection_kv_block_table.device().is_privateuseone(),
                "selection_kv_block_table must be on NPU.");
    TORCH_CHECK(selection_kv_block_status.device().is_privateuseone(),
                "selection_kv_block_status must be on NPU.");
    TORCH_CHECK(req_pool_entries.device().is_privateuseone(),
                "req_pool_entries must be on NPU.");
    TORCH_CHECK(selection_topk_indices.device().is_privateuseone(),
                "selection_topk_indices must be on NPU.");
    TORCH_CHECK(full_k_rope.device().is_privateuseone(),
                "full_k_rope must be on NPU.");
    TORCH_CHECK(full_kv_cache.device().is_privateuseone(),
                "full_kv_cache must be on NPU.");
    TORCH_CHECK(full_kv_block_table.device().is_privateuseone(),
                "full_kv_block_table must be on NPU.");
    TORCH_CHECK(full_kv_actual_seq.device().is_privateuseone(),
                "full_kv_actual_seq must be on NPU.");
    TORCH_CHECK(row_modes.device().is_privateuseone(),
                "row_modes must be on NPU.");
    TORCH_CHECK(budget_lengths.device().is_privateuseone(),
                "budget_lengths must be on NPU.");
    TORCH_CHECK(tail_valid_token_counts.device().is_privateuseone(),
                "tail_valid_token_counts must be on NPU.");
    TORCH_CHECK(resident_tail_starts.device().is_privateuseone(),
                "resident_tail_starts must be on NPU.");
    TORCH_CHECK(query_position_rows.device().is_privateuseone(),
                "query_position_rows must be on NPU.");
    TORCH_CHECK(attention_indices_out.device().is_privateuseone(),
                "attention_indices_out must be on NPU.");
    TORCH_CHECK(selection_kv_block_table.dim() == 2,
                "selection_kv_block_table must be [batch, blocks].");
    TORCH_CHECK(selection_kv_block_status.dim() == 4,
                "selection_kv_block_status must be "
                "[pool_capacity, 1, 1, topk+1].");
    TORCH_CHECK(req_pool_entries.dim() == 1,
                "req_pool_entries must be [batch].");
    ...
    TORCH_CHECK(full_kv_block_table.dim() == 2,
                "full_kv_block_table must be [batch, blocks].");
    TORCH_CHECK(full_kv_actual_seq.dim() == 1,
                "full_kv_actual_seq must be [batch].");
    TORCH_CHECK(budget_lengths.dim() == 1,
                "budget_lengths must be [batch].");
    TORCH_CHECK(tail_valid_token_counts.dim() == 1,
                "tail_valid_token_counts must be [batch].");
    TORCH_CHECK(resident_tail_starts.dim() == 1,
                "resident_tail_starts must be [batch].");
    TORCH_CHECK(query_position_rows.dim() == 2,
                "query_position_rows must be [batch, query_width].");
    TORCH_CHECK(attention_indices_out.dim() == 2 ||
                attention_indices_out.dim() == 3,
                "attention_indices_out must be [batch, width] or "
                "[batch, inner, width].");
    ...
    TORCH_CHECK(full_kv_block_table.size(0) == req_pool_entries.size(0),
                "full_kv_block_table batch must equal req_pool_entries.");
    TORCH_CHECK(full_kv_actual_seq.size(0) == req_pool_entries.size(0),
                "full_kv_actual_seq batch must equal req_pool_entries.");
    ...
    return std::make_tuple(selection_k_rope, selection_kv_cache,
                           selection_kv_block_table,
                           selection_kv_block_status,
                           attention_indices_out);
}
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        ...
        "Tensor selection_topk_indices, Tensor full_k_rope, "
        "Tensor full_kv_cache, Tensor full_kv_block_table, "
        "Tensor full_kv_actual_seq, Tensor row_modes, "
        "Tensor budget_lengths, Tensor tail_valid_token_counts, "
        "Tensor resident_tail_starts, Tensor query_position_rows, "
        "Tensor! attention_indices_out) "
