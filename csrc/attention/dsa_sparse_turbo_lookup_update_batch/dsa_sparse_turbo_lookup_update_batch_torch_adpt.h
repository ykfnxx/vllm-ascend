...
#ifndef DSA_SPARSE_LOOKUP_UPDATE_TURBO_TORCH_ADPT_H
#define DSA_SPARSE_LOOKUP_UPDATE_TURBO_TORCH_ADPT_H
    ...
    int64_t req_num)
{
    ...
    TORCH_CHECK(index.is_contiguous() &&
                ...
                "all dsa_sparse_turbo_lookup_update_batch tensors must be contiguous");
    ...
    TORCH_CHECK(slot_to_index.device() == device &&
                ...
                "all dsa_sparse_turbo_lookup_update_batch tensors must be on one device");

    at::Tensor slot_out = at::empty_like(query_index);
    at::Tensor miss_out = at::empty_like(query_index);
    EXEC_NPU_CMD(
        aclnnDsaSparseTurboLookupUpdateBatch,
        index,
        slot_to_index,
        free_slots,
        free_head,
...
#endif  // DSA_SPARSE_LOOKUP_UPDATE_TURBO_TORCH_ADPT_H
