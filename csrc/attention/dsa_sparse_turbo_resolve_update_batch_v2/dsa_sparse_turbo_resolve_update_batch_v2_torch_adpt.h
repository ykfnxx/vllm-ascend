...
#ifndef DSA_SPARSE_TURBO_RESOLVE_UPDATE_BATCH_V2_TORCH_ADPT_H
#define DSA_SPARSE_TURBO_RESOLVE_UPDATE_BATCH_V2_TORCH_ADPT_H
    ...
    int64_t decode_mode)
{
    ...
    TORCH_CHECK(index.scalar_type() == at::kInt &&
                ...
                "all turbo resolve-update V2 tensors must be int32");
    ...
    TORCH_CHECK(mapped_indices_out.sizes() == semantic_topk.sizes() &&
                ...
                "turbo resolve-update V2 outputs must match semantic_topk");
    TORCH_CHECK(index.is_contiguous() &&
                ...
                "all turbo resolve-update V2 tensors must be contiguous");
    const auto device = index.device();
    TORCH_CHECK(device.is_privateuseone(),
                "turbo resolve-update V2 state must be on NPU");
    TORCH_CHECK(slot_to_index.device() == device &&
                ...
                "all turbo resolve-update V2 tensors must be on one device");
...
#endif  // DSA_SPARSE_TURBO_RESOLVE_UPDATE_BATCH_V2_TORCH_ADPT_H
