...
static ge::graphStatus InferShapeForDsaSparseTurboFusedLookupUpdateBatch(
    gert::InferShapeContext* context)
{
    ...
    for (size_t dim = 0; dim < query_shape->GetDimNum(); ++dim) {
        ...
        mapped_shape->SetDim(dim, value);
        miss_mask_shape->SetDim(dim, value);
    }
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataTypeForDsaSparseTurboFusedLookupUpdateBatch(
...
IMPL_OP_INFERSHAPE(DsaSparseTurboFusedLookupUpdateBatch)
    .InferShape(InferShapeForDsaSparseTurboFusedLookupUpdateBatch)
    .InferDataType(InferDataTypeForDsaSparseTurboFusedLookupUpdateBatch);
