...
#ifndef DSA_SPARSE_LOOKUP_UPDATE_TURBO_FUSED_SIMT_H
#define DSA_SPARSE_LOOKUP_UPDATE_TURBO_FUSED_SIMT_H
...
namespace DsaSparseTurboFusedLookupUpdateBatch {
...
// Request-level maintain, identical to the turbo baseline: ONE victim-count
// scan and ONE eviction pass over the request's complete protection set,
// zero-atomic early exit, early return when nothing was allocated, LIFO
// refill and the overflow fallback that reverts trailing allocations.  The
// reverted allocations reset the fused outputs (mapped_indices ->
// fallback_slot, miss_mask -> 0) exactly like the framework's
// fallback_mask -> fallback_slot path.
    ...
    int32_t fallback_slot)
{
    ...
    for (uint32_t position = scan_begin;
        ...
        if (slot >= DSA_SPARSE_TURBO_FUSED_SLOT_COUNT) {
            slot -= DSA_SPARSE_TURBO_FUSED_SLOT_COUNT;
        }
    ...
    // reset and the fused outputs back to the framework-equivalent fallback
    // mapping (mapped_indices = fallback_slot, miss_mask = 0).  Runs only on
    ...
    if (effective < cum_alloc) {
        for (uint32_t rank = static_cast<uint32_t>(effective) + tid;
            ...
            request_slot_to_index[
                ...
                DSA_SPARSE_TURBO_FUSED_NOT_FOUND;
            mapped_indices[offset] = fallback_slot;
            miss_mask[offset] = 0;
        }
        asc_threadfence_block();
        asc_syncthreads();
...
// Fused classification + lookup + miss allocation for one query row.
//
// Compared with the turbo baseline (which consumed a framework-generated
// lookup_mask), this kernel classifies every Top-K token in-kernel using the
// request-level verify_start/tail_start (loaded once per request) and the
// query's current position:
//
...
// inherited unchanged from the turbo baseline.
...
#pragma unroll
    for (uint32_t local_entry = 0U;
        ...
        const uint64_t offset =
            query_base + query_begin + local_entry;
        // A history miss that could not be allocated (budget cap) maps to the
        // fallback slot, mirroring the framework's fallback_mask path.
        if (query_is_history[local_entry] != 0 &&
            local_miss_candidates[local_entry] != 0 &&
            local_misses[local_entry] == 0) {
            mapped_indices[offset] = fallback_slot;
            ...
            mapped_indices[offset] = local_mapped[local_entry];
...
#pragma unroll
        for (uint32_t local_entry = 0U;
            ...
            mapped_indices[offset] = kInvalidIndex;
            miss_mask[offset] = 0;
        }
    }
}
...
DsaSparseTurboFusedLookupUpdateBatchSimt(
    __gm__ int32_t* index,
    __gm__ int32_t* slot_to_index,
    __gm__ int32_t* free_slots,
    __gm__ int32_t* free_head,
...
#endif  // DSA_SPARSE_LOOKUP_UPDATE_TURBO_FUSED_SIMT_H
