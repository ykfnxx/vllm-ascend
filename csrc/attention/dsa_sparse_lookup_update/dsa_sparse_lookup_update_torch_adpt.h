...
inline std::tuple<at::Tensor, at::Tensor> dsa_sparse_lookup_update(
    at::Tensor& index,
    ...
    int64_t req_num)
{
    ...
    TORCH_CHECK(query_index.dim() == 2 &&
                    ...
                    query_index.size(1) == kQueryCount,
    ...
    TORCH_CHECK(index.is_contiguous() &&
                ...
                "all dsa_sparse_lookup_update tensors must be contiguous");
    ...
    TORCH_CHECK(slot_to_index.device() == device &&
                ...
                "all dsa_sparse_lookup_update tensors must be on one device");
