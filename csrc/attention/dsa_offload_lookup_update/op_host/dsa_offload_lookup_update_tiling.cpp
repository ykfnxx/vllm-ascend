...
#include "dsa_offload_lookup_update_tiling.h"
...
static ge::graphStatus DsaOffloadLookupUpdateTilingFunc(
    gert::TilingContext* context)
{
    if (context == nullptr) {
    ...
    auto* tiling_data =
        context->GetTilingData<DsaOffloadLookupUpdateTilingData>();
...
static ge::graphStatus TilingParseForDsaOffloadLookupUpdate(
...
IMPL_OP_OPTILING(DsaOffloadLookupUpdate)
    .Tiling(DsaOffloadLookupUpdateTilingFunc)
    .TilingParse<DsaOffloadLookupUpdateCompileInfo>(
        TilingParseForDsaOffloadLookupUpdate);
