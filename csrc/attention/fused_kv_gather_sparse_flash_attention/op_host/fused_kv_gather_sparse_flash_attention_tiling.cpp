...
#include "fused_kv_gather_sparse_flash_attention_tiling.h"
...
void SFAMlaTiling::FillTilingBaseParamsMla()
{
    ...
    tilingData_.baseParams.set_dsaRouteWidth(sfaInfo_->dsaRouteWidth);
    tilingData_.baseParams.set_dsaHotTableWidth(sfaInfo_->dsaHotTableWidth);
    tilingData_.baseParams.set_dsaHostTableWidth(sfaInfo_->dsaHostTableWidth);
    tilingData_.baseParams.set_dsaHotPhysicalBlocks(sfaInfo_->dsaHotPhysicalBlocks);
    tilingData_.baseParams.set_dsaHostPhysicalBlocks(sfaInfo_->dsaHostPhysicalBlocks);
    tilingData_.baseParams.set_dsaHotPoolCapacity(sfaInfo_->dsaHotPoolCapacity);
    tilingData_.baseParams.set_dsaHostPoolCapacity(sfaInfo_->dsaHostPoolCapacity);
}
...
namespace {
bool ReadFusedShape(const gert::TilingContext *context, uint32_t input,
                    uint32_t rank, int64_t *dims)
{
    const auto *shape = context->GetInputShape(input);
    if (shape == nullptr) {
        return false;
    }
    const auto &storage = shape->GetStorageShape();
    if (storage.GetDimNum() != rank) {
        return false;
    }
    for (uint32_t dim = 0; dim < rank; ++dim) {
        dims[dim] = storage.GetDim(dim);
        if (dims[dim] <= 0 || static_cast<uint64_t>(dims[dim]) >
...
ge::graphStatus ParseFusedDsaInputs(gert::TilingContext *context,
                                    SFATilingInfo &sfaInfo)
{
    int64_t hotKey[4], hotRope[4], hotTable[2];
    int64_t hostKey[3], hostRope[3], hostTable[2];
    int64_t queryPool[1], hotRows[1], route[2], query[3], sparse[3];
    int64_t stagingKey[4], stagingRope[4];
    if (!ReadFusedShape(context, KEY_INPUT_INDEX, 4U, hotKey) ||
        !ReadFusedShape(context, KEY_ROPE_INPUT_INDEX, 4U, hotRope) ||
        !ReadFusedShape(context, BLOCK_TABLE_INPUT_INDEX, 2U, hotTable) ||
        !ReadFusedShape(context, HOST_KEY_INPUT_INDEX, 3U, hostKey) ||
        !ReadFusedShape(context, HOST_KEY_ROPE_INPUT_INDEX, 3U, hostRope) ||
        !ReadFusedShape(context, HOST_BLOCK_TABLE_INPUT_INDEX, 2U, hostTable) ||
        !ReadFusedShape(context, QUERY_POOL_ENTRIES_INPUT_INDEX, 1U, queryPool) ||
        !ReadFusedShape(context, HOT_SOURCE_ROWS_INPUT_INDEX, 1U, hotRows) ||
        !ReadFusedShape(context, ROUTE_PLAN_INPUT_INDEX, 2U, route) ||
        !ReadFusedShape(context, INSTALL_STAGING_KEY_INPUT_INDEX, 4U,
                        stagingKey) ||
        !ReadFusedShape(context, INSTALL_STAGING_ROPE_INPUT_INDEX, 4U,
                        stagingRope) ||
        !ReadFusedShape(context, QUERY_INPUT_INDEX, 3U, query) ||
        !ReadFusedShape(context, SPARSE_INDICES_INPUT_INDEX, 3U, sparse)) {
        OP_LOGE("FusedKvGatherSparseFlashAttention", "fused DSA input rank or dimension is invalid");
        return ge::GRAPH_FAILED;
    }
    const bool shapeValid =
        hotKey[0] == hotRope[0] && hotKey[1] == hotRope[1] &&
        hostKey[0] == hostRope[0] && hostKey[1] == hostRope[1] &&
        hotKey[1] == hostKey[1] && hotKey[2] == 1 && hotRope[2] == 1 &&
        hotKey[3] == hostKey[2] && hotRope[3] == hostRope[2] &&
        query[0] == route[0] && queryPool[0] == route[0] &&
        hotRows[0] == route[0] && sparse[0] == route[0] &&
        sparse[1] == 1 && sparse[2] == route[1];
    const uint64_t stagingTokens = static_cast<uint64_t>(stagingKey[0]) *
        static_cast<uint64_t>(stagingKey[1]);
    const uint64_t routeTokens = static_cast<uint64_t>(route[0]) *
        static_cast<uint64_t>(route[1]);
    const bool stagingValid =
        stagingKey[0] == stagingRope[0] &&
        stagingKey[1] == hotKey[1] && stagingRope[1] == hotKey[1] &&
        stagingKey[2] == 1 && stagingRope[2] == 1 &&
        stagingKey[3] == hotKey[3] && stagingRope[3] == hotRope[3] &&
        stagingTokens >= routeTokens;
    if (!shapeValid || !stagingValid) {
        OP_LOGE("FusedKvGatherSparseFlashAttention", "Hot/Host/route shapes are inconsistent");
        return ge::GRAPH_FAILED;
    }
    const auto *hotKeyDesc = context->GetInputDesc(KEY_INPUT_INDEX);
    const auto *hotRopeDesc = context->GetInputDesc(KEY_ROPE_INPUT_INDEX);
    const auto *hostKeyDesc = context->GetInputDesc(HOST_KEY_INPUT_INDEX);
    const auto *hostRopeDesc = context->GetInputDesc(HOST_KEY_ROPE_INPUT_INDEX);
    const auto *stagingKeyDesc = context->GetInputDesc(
        INSTALL_STAGING_KEY_INPUT_INDEX);
    const auto *stagingRopeDesc = context->GetInputDesc(
        INSTALL_STAGING_ROPE_INPUT_INDEX);
    if (hotKeyDesc == nullptr || hotRopeDesc == nullptr || hostKeyDesc == nullptr ||
        hostRopeDesc == nullptr || stagingKeyDesc == nullptr ||
        stagingRopeDesc == nullptr ||
        hotKeyDesc->GetDataType() != hostKeyDesc->GetDataType() ||
        hotRopeDesc->GetDataType() != hostRopeDesc->GetDataType() ||
        hotKeyDesc->GetDataType() != hotRopeDesc->GetDataType() ||
        hotKeyDesc->GetDataType() != stagingKeyDesc->GetDataType() ||
        hotRopeDesc->GetDataType() != stagingRopeDesc->GetDataType()) {
        OP_LOGE("FusedKvGatherSparseFlashAttention", "Hot and Host cache dtypes must match");
        return ge::GRAPH_FAILED;
    }
    constexpr int64_t DSA_BLOCK_SIZE = 128;
    if (sfaInfo.qLayout != SFALayout::TND || !sfaInfo.pageAttentionFlag ||
        sfaInfo.n2Size != 1U || sfaInfo.sparseBlockSize != 1 ||
        hotKey[1] != sfaInfo.blockSize || hotKey[1] != DSA_BLOCK_SIZE ||
        hotKey[3] != 512 || hotRope[3] != 64) {
        OP_LOGE("FusedKvGatherSparseFlashAttention",
                "only TND/PA_BSND MLA D=512, rope=64, N=1, block_size=128, sparse_block_size=1 is supported");
        return ge::GRAPH_FAILED;
    }
    sfaInfo.dsaRouteWidth = static_cast<uint32_t>(route[1]);
    sfaInfo.dsaHotTableWidth = static_cast<uint32_t>(hotTable[1]);
    sfaInfo.dsaHostTableWidth = static_cast<uint32_t>(hostTable[1]);
    sfaInfo.dsaHotPhysicalBlocks = static_cast<uint32_t>(hotKey[0]);
    sfaInfo.dsaHostPhysicalBlocks = static_cast<uint32_t>(hostKey[0]);
    sfaInfo.dsaHotPoolCapacity = static_cast<uint32_t>(hotTable[0]);
    sfaInfo.dsaHostPoolCapacity = static_cast<uint32_t>(hostTable[0]);
    return ge::GRAPH_SUCCESS;
}
}  // namespace

ge::graphStatus TilingFusedKvGatherSparseFlashAttention(gert::TilingContext *context)
{
    SFATilingInfo sfaInfo;
    ...
    if (ParseFusedDsaInputs(context, sfaInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus TilingPrepareForFusedKvGatherSparseFlashAttention(gert::TilingParseContext* const context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SFATilingCheck::CheckBlockTable() const
{
    ...
    // Route-aware SFA does not index the Hot table with the virtual TND
    // batch.  Each query row first resolves hot_source_rows[b] and then uses
    // that persistent pool row.  ParseFusedDsaInputs validates the non-empty
    // table and exports its first dimension as dsaHotPoolCapacity, while the
    // kernel independently bounds-checks every resolved source row.
...
IMPL_OP_OPTILING(FusedKvGatherSparseFlashAttention)
    .Tiling(TilingFusedKvGatherSparseFlashAttention)
    .TilingParse<SparseFlashAttentionCompileInfo>(TilingPrepareForFusedKvGatherSparseFlashAttention);
} // namespace fused_kv_gather_sfa
} // namespace optiling
