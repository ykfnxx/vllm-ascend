...
public:
    ...
    __aicore__ inline void CopyOutMrgeResult(
        int64_t mte2Size, int64_t mte3Size, int64_t s2StartGmOffset,
        int64_t mergeMte3Idx, int64_t topkGmBaseOffset,
        const RunInfo &runInfo);
    ...
    __aicore__ inline void CopyInRouteSingle(int64_t &mte2Size, int64_t mte3Size,
                                             int64_t mergeMte3Idx, int64_t packedRoute);
    // ================================Vector1==========================================
...
private:
    ...
    // Vec0 batches one route tile in a dedicated buffer.  The dedicated
    // allocation avoids races with tmpBuff1, which Vec1/Vec2 reuse.
    static constexpr uint32_t DSA_ROUTE_TILE_ELEMENTS = 512U;
    static constexpr uint32_t DSA_BLOCK_SIZE = 128U;
    static constexpr uint32_t DSA_BLOCK_SIZE_SHIFT = 7U;
    static constexpr uint32_t DSA_BLOCK_OFFSET_MASK = DSA_BLOCK_SIZE - 1U;
    static_assert(
        DSA_ROUTE_TILE_ELEMENTS * sizeof(int32_t) <=
            ConstInfo::BUFFER_SIZE_BYTE_32K,
        "DSA route tile must fit in its dedicated UB buffer");
    ...
    GlobalTensor<KV_T> dsaHotKeyGm_;
    GlobalTensor<KV_T> dsaHotRopeGm_;
    GlobalTensor<KV_T> dsaHostKeyGm_;
    GlobalTensor<KV_T> dsaHostRopeGm_;
    GlobalTensor<int32_t> dsaHotBlockTableGm_;
    GlobalTensor<int32_t> dsaHostBlockTableGm_;
    GlobalTensor<int32_t> dsaQueryPoolEntriesGm_;
    GlobalTensor<int32_t> dsaHotSourceRowsGm_;
    GlobalTensor<int32_t> dsaRoutePlanGm_;
    GlobalTensor<KV_T> dsaInstallStagingKeyGm_;
    GlobalTensor<KV_T> dsaInstallStagingRopeGm_;
    bool dsaRouteEnabled_ = false;
    int32_t dsaHotSourceRow_ = -1;
    int32_t dsaHostSourceRow_ = -1;
    uint64_t dsaHotTableRowBase_ = 0U;
    uint64_t dsaHostTableRowBase_ = 0U;
    uint32_t dsaHotCachedLogicalBlock_ = UINT32_MAX;
    uint32_t dsaHostCachedLogicalBlock_ = UINT32_MAX;
    int32_t dsaHotCachedPhysicalBlock_ = -1;
    int32_t dsaHostCachedPhysicalBlock_ = -1;
    ...
    TBuf<> dsaRouteBuff;          // 2K, Vec0-only route tile
    ...
    LocalTensor<int32_t> dsaRouteUb_;
};
...
template <typename SFAT> __aicore__ inline void SFAVectorService<SFAT>::InitBuffers(TPipe *pipe)
{
    ...
    pipe->InitBuffer(dsaRouteBuff, DSA_ROUTE_TILE_ELEMENTS * sizeof(int32_t));
    ...
    dsaRouteUb_ = dsaRouteBuff.Get<int32_t>();
}
...
template <typename SFAT>
__aicore__ inline void SFAVectorService<SFAT>::InitDsaSources(
    ...
    __gm__ uint8_t *installStagingRope)
{
    dsaHotKeyGm_.SetGlobalBuffer(reinterpret_cast<__gm__ KV_T *>(hotKey));
    dsaHotRopeGm_.SetGlobalBuffer(reinterpret_cast<__gm__ KV_T *>(hotKeyRope));
    dsaHostKeyGm_.SetGlobalBuffer(reinterpret_cast<__gm__ KV_T *>(hostKey));
    dsaHostRopeGm_.SetGlobalBuffer(reinterpret_cast<__gm__ KV_T *>(hostKeyRope));
    dsaHotBlockTableGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(hotBlockTable));
    dsaHostBlockTableGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(hostBlockTable));
    dsaQueryPoolEntriesGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(queryPoolEntries));
    dsaHotSourceRowsGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(hotSourceRows));
    dsaRoutePlanGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(routePlan));
    dsaInstallStagingKeyGm_.SetGlobalBuffer(
        reinterpret_cast<__gm__ KV_T *>(installStagingKey));
    dsaInstallStagingRopeGm_.SetGlobalBuffer(
        reinterpret_cast<__gm__ KV_T *>(installStagingRope));
    dsaRouteEnabled_ = true;
}
                                                            ...
                                                            int64_t topkGmBaseOffset, const RunInfo &runInfo)
{
    ...
    if (dsaRouteEnabled_) {
        const int32_t packed = dsaRoutePlanGm_.GetValue(topkGmBaseOffset + topkGmIdx);
        const uint32_t kind = static_cast<uint32_t>(packed) >> 29U;
        realS2Idx = kind == 0U ? -1 : static_cast<int64_t>(static_cast<uint32_t>(packed));
    } else {
...
template <typename SFAT>
__aicore__ inline void SFAVectorService<SFAT>::CopyInRouteSingle(
    ...
    int64_t packedRoute)
{
    ...
    const int32_t sourceRow = useHot ? dsaHotSourceRow_ : dsaHostSourceRow_;
    if (sourceRow < 0 || static_cast<uint32_t>(sourceRow) >= poolCapacity) {
        return;
    }
    const uint32_t logicalBlock = sourcePosition >> DSA_BLOCK_SIZE_SHIFT;
    if (logicalBlock >= tableWidth) {
        return;
    }
    uint32_t &cachedLogicalBlock = useHot ? dsaHotCachedLogicalBlock_
                                          : dsaHostCachedLogicalBlock_;
    int32_t &cachedPhysicalBlock = useHot ? dsaHotCachedPhysicalBlock_
                                          : dsaHostCachedPhysicalBlock_;
    if (cachedLogicalBlock != logicalBlock) {
        const uint64_t tableRowBase = useHot ? dsaHotTableRowBase_
                                             : dsaHostTableRowBase_;
        cachedPhysicalBlock = useHot
            ? dsaHotBlockTableGm_.GetValue(tableRowBase + logicalBlock)
            : dsaHostBlockTableGm_.GetValue(tableRowBase + logicalBlock);
        cachedLogicalBlock = logicalBlock;
    }
    if (cachedPhysicalBlock < 0 ||
        static_cast<uint32_t>(cachedPhysicalBlock) >= physicalBlocks) {
        return;
    }
    const uint64_t sourceToken =
        (static_cast<uint64_t>(cachedPhysicalBlock) << DSA_BLOCK_SIZE_SHIFT) +
        (sourcePosition & DSA_BLOCK_OFFSET_MASK);
    DataCopyExtParams params;
    params.blockCount = 1;
    params.blockLen = constInfo.headDim * sizeof(KV_T);
    params.srcStride = 0;
    params.dstStride = 0;
    DataCopyPadExtParams<KV_T> padParams;
    const uint64_t dstKv = mergeMte3Idx % 2 * 32 * 512 +
                           (mte2Size - mte3Size) * constInfo.headDim;
    const uint64_t dstRope = mergeMte3Idx % 2 * 32 * 64 +
                             (mte2Size - mte3Size) * constInfo.headDimRope;
    if (useHot) {
        DataCopyPad(kvMergUb_[dstKv], dsaHotKeyGm_[sourceToken * constInfo.headDim], params, padParams);
    } else {
        DataCopyPad(kvMergUb_[dstKv], dsaHostKeyGm_[sourceToken * constInfo.headDim], params, padParams);
    }
    params.blockLen = constInfo.headDimRope * sizeof(KV_T);
    if (useHot) {
        DataCopyPad(ropeMergUb_[dstRope], dsaHotRopeGm_[sourceToken * constInfo.headDimRope], params, padParams);
    } else {
        DataCopyPad(ropeMergUb_[dstRope], dsaHostRopeGm_[sourceToken * constInfo.headDimRope], params, padParams);
    }
    ++mte2Size;
}
...
{
    if (dsaRouteEnabled_) {
        CopyInRouteSingle(mte2Size, mte3Size, mergeMte3Idx, realS2Idx1);
        CopyInRouteSingle(mte2Size, mte3Size, mergeMte3Idx, realS2Idx2);
        return;
    }
                                                                 ...
                                                                 const RunInfo &runInfo)
{
    ...
    constexpr uint32_t ROUTE_KIND_SHIFT = 29U;
    constexpr uint32_t SRC_HOST_MISS = 2U;
    const int64_t copied = mte2Size - mte3Size;
    const int64_t routeTileBase =
        runInfo.s2Idx * constInfo.s2BaseSize + s2GmStartOffset;
    for (int64_t index = 0; index < copied; ++index) {
        const int64_t routeIndex = mte3Size + index;
        const uint32_t kind = static_cast<uint32_t>(
            dsaRouteUb_.GetValue(routeIndex)) >> ROUTE_KIND_SHIFT;
        if (kind != SRC_HOST_MISS) {
            continue;
        }
        const uint64_t stagingToken = static_cast<uint64_t>(
            topkGmBaseOffset + routeTileBase + routeIndex);
        DataCopy(dsaInstallStagingKeyGm_[stagingToken * constInfo.headDim],
                 kvMergUb_[mergeMte3Idx % 2 * 32 * 512 +
                           index * constInfo.headDim],
                 constInfo.headDim);
        DataCopy(dsaInstallStagingRopeGm_[
                     stagingToken * constInfo.headDimRope],
                 ropeMergUb_[mergeMte3Idx % 2 * 32 * 64 +
                             index * constInfo.headDimRope],
                 constInfo.headDimRope);
    }
}
...
__aicore__ inline void SFAVectorService<SFAT>::MergeKv(const RunInfo &runInfo)
{
    ...
    if (dsaRouteEnabled_ && s2GmStartOffset < s2GmLimit) {
        const uint32_t routeStart = static_cast<uint32_t>(
            (runInfo.s2Idx * constInfo.s2BaseSize + s2GmStartOffset) /
            constInfo.sparseBlockSize);
        const uint32_t routeEnd = static_cast<uint32_t>(CeilDiv(
            runInfo.s2Idx * constInfo.s2BaseSize + s2GmLimit,
            static_cast<int64_t>(constInfo.sparseBlockSize)));
        const uint32_t routeCount = routeEnd - routeStart;
        DataCopyExtParams metadataCopyParams{
            1U, static_cast<uint32_t>(routeCount * sizeof(int32_t)),
            0U, 0U, 0U};
        DataCopyPadExtParams<int32_t> metadataPadParams;
        DataCopyPad(dsaRouteUb_,
                    dsaRoutePlanGm_[topkGmBaseOffset + routeStart],
                    metadataCopyParams, metadataPadParams);

        dsaHostSourceRow_ = dsaQueryPoolEntriesGm_.GetValue(runInfo.bIdx);
        dsaHotSourceRow_ = dsaHotSourceRowsGm_.GetValue(runInfo.bIdx);
        SetFlag<AscendC::HardEvent::MTE2_S>(0);
        WaitFlag<AscendC::HardEvent::MTE2_S>(0);
        dsaHotTableRowBase_ = dsaHotSourceRow_ >= 0
            ? static_cast<uint64_t>(dsaHotSourceRow_) *
                tilingData->baseParams.dsaHotTableWidth : 0U;
        dsaHostTableRowBase_ = dsaHostSourceRow_ >= 0
            ? static_cast<uint64_t>(dsaHostSourceRow_) *
                tilingData->baseParams.dsaHostTableWidth : 0U;
        dsaHotCachedLogicalBlock_ = UINT32_MAX;
        dsaHostCachedLogicalBlock_ = UINT32_MAX;
    }
    for (int64_t s2GmOffsetArray = s2GmStartOffset; s2GmOffsetArray < s2GmLimit; s2GmOffsetArray += 2 * constInfo.sparseBlockSize) {
        ...
        if (dsaRouteEnabled_) {
            const uint32_t localRouteIdx = static_cast<uint32_t>(
                (s2GmOffsetArray - s2GmStartOffset) /
                constInfo.sparseBlockSize);
            const int32_t packed = dsaRouteUb_.GetValue(localRouteIdx);
            const uint32_t kind = static_cast<uint32_t>(packed) >> 29U;
            s2IdxArray0 = kind == 0U ? -1 :
                static_cast<int64_t>(static_cast<uint32_t>(packed));
        } else {
            GetRealS2Idx(s2GmOffsetArray, s2IdxArray0,
                         topkGmBaseOffset, runInfo);
        }
        if (unlikely(s2IdxArray0 < 0)) {
            CopyOutMrgeResult(mte2Size, mte3Size, s2GmStartOffset,
                              mergeMte3Idx, topkGmBaseOffset, runInfo);
            SetFlag<AscendC::HardEvent::MTE3_MTE2>(mergeMte3Idx % 2);
            mergeMte3Idx++;
            break;
        }
        if (dsaRouteEnabled_) {
            const int64_t secondOffset =
                s2GmOffsetArray + constInfo.sparseBlockSize;
            if (secondOffset < s2GmLimit) {
                const uint32_t localRouteIdx = static_cast<uint32_t>(
                    (secondOffset - s2GmStartOffset) /
                    constInfo.sparseBlockSize);
                const int32_t packed = dsaRouteUb_.GetValue(localRouteIdx);
                const uint32_t kind =
                    static_cast<uint32_t>(packed) >> 29U;
                s2IdxArray1 = kind == 0U ? -1 :
                    static_cast<int64_t>(static_cast<uint32_t>(packed));
            } else {
                s2IdxArray1 = -1;
            }
