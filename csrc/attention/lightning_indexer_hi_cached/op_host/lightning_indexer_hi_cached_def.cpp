...
namespace ops {
class LightningIndexerHiCached : public OpDef {
public:
    explicit LightningIndexerHiCached(const char *name) : OpDef(name)
    {
        this->Input("query")
            .ParamType(REQUIRED)
        ...
        this->Attr("hi_block_size").AttrType(OPTIONAL).Int(128);
        this->Attr("hi_block_num").AttrType(OPTIONAL).Int(64);
        this->Attr("sink").AttrType(OPTIONAL).Int(1);
        this->Attr("recent").AttrType(OPTIONAL).Int(2);
        this->Attr("block_pooling_mode").AttrType(OPTIONAL).String("mean");
...
OP_ADD(LightningIndexerHiCached);
} // namespace ops
