...
#ifndef LIGHTNING_INDEXER_HI_CACHED_ARCH32_KERNEL_H
#define LIGHTNING_INDEXER_HI_CACHED_ARCH32_KERNEL_H
...
public:
    ...
    static constexpr uint32_t S1_BASE_SIZE = 8;
...
__aicore__ inline void LIPreload<LIT>::InitTilingData(const LIHiCachedTilingData *__restrict tilingData)
{
    ...
    // Keep the S1 tile at 8 tokens for topk <= 2048. The Cube M tile is
    // therefore 512 for ratio 64 and 256 for ratio 32.
...
__aicore__ inline bool LIPreload<LIT>::RunDecodeStage2HiFastPath()
{
    ...
    for (uint32_t taskIdx = aiCoreIdx; taskIdx < totalTaskNum; taskIdx += GetBlockNum()) {
        ...
        // Split one row's fixed HI block list into 512-token chunks. When
...
__aicore__ inline void LIPreload<LIT>::RunStage2()
{
    ...
    if constexpr (LAYOUT_T == LI_LAYOUT::TND && LIT::pageAttention) {
        ...
        bool useFastPathTopology =
            decodeStage2HiFastPathEligible && totalRowNum > 0 && GetBlockNum() >= totalRowNum;
...
#endif // LIGHTNING_INDEXER_HI_CACHED_ARCH32_KERNEL_H
