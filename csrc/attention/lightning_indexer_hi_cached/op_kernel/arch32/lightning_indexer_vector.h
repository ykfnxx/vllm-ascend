 ...
 * \brief
 */
#ifndef LIGHTNING_INDEXER_HI_CACHED_ARCH32_VECTOR_H
#define LIGHTNING_INDEXER_HI_CACHED_ARCH32_VECTOR_H
...
__aicore__ inline void MergeSortTopN(const LocalTensor<float> &mrgDst, int32_t mrgDstValidNum,
                                     LocalTensor<float> &mrgSrc, int32_t mrgSrcNum, int32_t outNum,
                                     LocalTensor<float> &tmpTensor)
{
    AscendC::MrgSort4Info params;
    params.elementLengths[0] = mrgDstValidNum;
    params.elementLengths[1] = mrgSrcNum;
    params.ifExhaustedSuspension = false;
    params.validBit = 0b0011;
    params.repeatTimes = 1;
    ...
    AscendC::MrgSort<float>(tmpTensor, srcList, params);
    AscendC::PipeBarrier<PIPE_V>();
    AscendC::DataCopy(mrgDst, tmpTensor, outNum * VALUE_AND_INDEX_NUM);
    AscendC::PipeBarrier<PIPE_V>();
}
...
#endif // LIGHTNING_INDEXER_HI_CACHED_ARCH32_VECTOR_H
