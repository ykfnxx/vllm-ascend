...
#define INVOKE_LI_NO_KFC_OP_IMPL(templateClass, ...)                                                                   \
    do {                                                                                                               \
        ...
        LI_COPY_TILING_DATA(LIHiCachedTilingData, tiling);                                                                   \
        op.Init(query, key, weights, actualSeqLengthsQ, actualSeqLengths, blocktable, stage1MeanCache,                \
...
__global__ __aicore__ void lightning_indexer_hi_cached(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
