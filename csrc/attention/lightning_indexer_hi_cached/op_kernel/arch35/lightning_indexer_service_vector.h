...
public:
    ...
    static constexpr int32_t REGBASE_MAX_TOPK = BASE_TOPK;
    static constexpr int32_t REGBASE_MAX_CANDIDATES = BASE_TOPK * 2;
...
private:
    ...
    TBuf<TPosition::VECCALC> regBaseScoreBuf_;
    TBuf<TPosition::VECCALC> regBaseKeyBuf_;
    TBuf<TPosition::VECCALC> regBaseRealIndexBuf_;
    ...
    __aicore__ inline void InitTopKList(const LocalTensor<float> &list, int32_t capacity);
    __aicore__ inline void InitTopKLists(const LocalTensor<float> &lists, int32_t capacity, int32_t listCount);
    __aicore__ inline void CopyTopKList(const LocalTensor<float> &dst, int32_t dstCapacity,
                                       const LocalTensor<float> &src, int32_t srcCapacity, int32_t count);
    __aicore__ inline LocalTensor<int32_t> GetTopKIndices(const LocalTensor<float> &list, int32_t capacity);
    __aicore__ inline int32_t GetTopKIndex(const LocalTensor<float> &list, int32_t capacity, int32_t pos);
    ...
    __aicore__ inline void InitRegBaseTopKBuffers(TPipe *pipe);
    __aicore__ inline void FinalizeTopKList(const LocalTensor<float> &dst, int32_t inputCount,
                                            int32_t topCount, int32_t outputCapacity);
    __aicore__ inline void SelectTopKList(const LocalTensor<float> &dst,
                                          const LocalTensor<float> &scores,
                                          const LocalTensor<int32_t> &realIndices,
                                          int32_t inputLen, int32_t topCount,
                                          int32_t outputCapacity);
    __aicore__ inline void SelectTileTopK(const LocalTensor<float> &dst,
                                          const LocalTensor<float> &scores,
                                          const LocalTensor<int32_t> &realIndices,
                                          int32_t inputLen, int32_t topCount);
    __aicore__ inline void MergeTopKLists(const LocalTensor<float> &dst, int32_t dstCapacity,
                                          ...
                                          int32_t firstCount,
                                          const LocalTensor<float> &second, int32_t secondCapacity,
                                          int32_t secondCount, int32_t topCount);
};
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::InitRegBaseTopKBuffers(TPipe *pipe)
{
    pipe->InitBuffer(regBaseScoreBuf_, REGBASE_MAX_CANDIDATES * sizeof(float));
    pipe->InitBuffer(regBaseKeyBuf_, REGBASE_MAX_CANDIDATES * sizeof(uint16_t));
    pipe->InitBuffer(regBaseRealIndexBuf_, REGBASE_MAX_CANDIDATES * sizeof(int32_t));
    pipe->InitBuffer(regBaseOutputIdxBuf_,
                     topk::LITopk::GetOutputIndexBufferSize(REGBASE_MAX_TOPK));
    pipe->InitBuffer(regBaseTopkTmpBuf_,
                     topk::LITopk::GetSharedTmpBufferSize(REGBASE_MAX_TOPK, REGBASE_MAX_CANDIDATES));
}

template <typename LIT>
__aicore__ inline void LIVector<LIT>::InitTopKList(const LocalTensor<float> &list, int32_t capacity)
{
    Duplicate(list.template ReinterpretCast<int32_t>(), LIServiceVec::NEG_INF, capacity);
    Duplicate(GetTopKIndices(list, capacity), LIServiceVec::INVALID_INDEX, capacity);
    PipeBarrier<PIPE_V>();
}

template <typename LIT>
__aicore__ inline void LIVector<LIT>::InitTopKLists(const LocalTensor<float> &lists,
                                                    int32_t capacity, int32_t listCount)
{
    for (int32_t i = 0; i < listCount; ++i) {
        InitTopKList(lists[i * capacity * TOPK_LIST_COMPONENTS], capacity);
    }
}

template <typename LIT>
__aicore__ inline LocalTensor<int32_t> LIVector<LIT>::GetTopKIndices(const LocalTensor<float> &list,
                                                                    int32_t capacity)
{
    return list[capacity].template ReinterpretCast<int32_t>();
}

template <typename LIT>
__aicore__ inline int32_t LIVector<LIT>::GetTopKIndex(const LocalTensor<float> &list,
                                                      int32_t capacity, int32_t pos)
{
    return GetTopKIndices(list, capacity).GetValue(pos);
}

template <typename LIT>
__aicore__ inline void LIVector<LIT>::CopyTopKList(const LocalTensor<float> &dst, int32_t dstCapacity,
                                                   const LocalTensor<float> &src, int32_t srcCapacity,
                                                   int32_t count)
{
    InitTopKList(dst, dstCapacity);
    int32_t copyCount = LICommon::Min(count, LICommon::Min(dstCapacity, srcCapacity));
    if (copyCount <= 0) {
        return;
    }
    Adds(dst, src, 0.0f, copyCount);
    Adds(GetTopKIndices(dst, dstCapacity), GetTopKIndices(src, srcCapacity), 0, copyCount);
    PipeBarrier<PIPE_V>();
}

template <typename LIT>
__aicore__ inline void LIVector<LIT>::FinalizeTopKList(const LocalTensor<float> &dst,
                                                       int32_t inputCount, int32_t topCount,
                                                       int32_t outputCapacity)
{
    InitTopKList(dst, outputCapacity);
    int32_t selectedCount = LICommon::Min(outputCapacity, LICommon::Min(inputCount, topCount));
    if (selectedCount <= 0) {
        return;
    }

    int32_t alignedInputCount = LICommon::Align(inputCount, 256);
    LocalTensor<float> candidateScores = regBaseScoreBuf_.Get<float>();
    LocalTensor<uint16_t> keys = regBaseKeyBuf_.Get<uint16_t>();
    LocalTensor<int32_t> realIndices = regBaseRealIndexBuf_.Get<int32_t>();
    Cast(keys.template ReinterpretCast<bfloat16_t>(), candidateScores, RoundMode::CAST_ROUND, alignedInputCount);
    PipeBarrier<PIPE_V>();
    topk::BuildSortableKeys(keys, alignedInputCount);
    PipeBarrier<PIPE_V>();
    ...
    // half as the native LI selected-value output without adding another UB.
    LocalTensor<uint16_t> selectedValues = candidateScores.template ReinterpretCast<uint16_t>();
    regBaseTileTopk_(selectedPositions, selectedValues, keys, alignedInputCount);
    PipeBarrier<PIPE_V>();

    topk::RestoreBfloat16Scores(selectedValues, LICommon::Align(selectedCount, 128));
    PipeBarrier<PIPE_V>();
    Cast(dst, selectedValues.template ReinterpretCast<bfloat16_t>(), RoundMode::CAST_NONE, selectedCount);
    PipeBarrier<PIPE_V>();
    int32_t vectorCount = (selectedCount / 64) * 64;
    if (vectorCount > 0) {
        topk::GatherRealIndices(GetTopKIndices(dst, outputCapacity), selectedPositions,
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::SelectTopKList(const LocalTensor<float> &dst,
                                                     const LocalTensor<float> &scores,
                                                     const LocalTensor<int32_t> &realIndices,
                                                     int32_t inputLen, int32_t topCount,
                                                     int32_t outputCapacity)
{
    int32_t alignedInputCount = LICommon::Align(inputLen, 256);
    LocalTensor<float> candidateScores = regBaseScoreBuf_.Get<float>();
    LocalTensor<int32_t> candidateIndices = regBaseRealIndexBuf_.Get<int32_t>();
    Duplicate(candidateScores.template ReinterpretCast<int32_t>(), LIServiceVec::NEG_INF, alignedInputCount);
    Duplicate(candidateIndices, LIServiceVec::INVALID_INDEX, alignedInputCount);
    PipeBarrier<PIPE_V>();
    Adds(candidateScores, scores, 0.0f, inputLen);
    Adds(candidateIndices, realIndices, 0, inputLen);
    PipeBarrier<PIPE_V>();
    FinalizeTopKList(dst, inputLen, topCount, outputCapacity);
}

template <typename LIT>
__aicore__ inline void LIVector<LIT>::SelectTileTopK(const LocalTensor<float> &dst,
                                                     const LocalTensor<float> &scores,
                                                     const LocalTensor<int32_t> &realIndices,
                                                     int32_t inputLen, int32_t topCount)
{
...
template <typename LIT>
__aicore__ inline void LIVector<LIT>::MergeTopKLists(const LocalTensor<float> &dst, int32_t dstCapacity,
                                                     const LocalTensor<float> &first, int32_t firstCapacity,
                                                     int32_t firstCount,
                                                     const LocalTensor<float> &second, int32_t secondCapacity,
                                                     int32_t secondCount, int32_t topCount)
{
    firstCount = LICommon::Min(firstCount, firstCapacity);
    secondCount = LICommon::Min(secondCount, secondCapacity);
    // A5 vector sources must start at a 32-byte-aligned UB address. Stage1
    // streaming TopM can produce non-aligned counts (for example 29 for
    // hi_block_num=32, sink=1, recent=2), so do not append the second list at
    // candidateScores[firstCount]. Keep the padding invalid and include it in
    // the VF input span while limiting TopK by the real candidate count.
    int32_t secondOffset = LICommon::Align(firstCount, static_cast<int32_t>(B32_BLOCK_ALIGN_NUM));
    int32_t inputCount = secondOffset + secondCount;
    int32_t realCandidateCount = firstCount + secondCount;
    int32_t alignedInputCount = LICommon::Align(inputCount, 256);
    LocalTensor<float> candidateScores = regBaseScoreBuf_.Get<float>();
    LocalTensor<int32_t> candidateIndices = regBaseRealIndexBuf_.Get<int32_t>();
    Duplicate(candidateScores.template ReinterpretCast<int32_t>(), LIServiceVec::NEG_INF, alignedInputCount);
    ...
    PipeBarrier<PIPE_V>();
    if (firstCount > 0) {
        Adds(candidateScores, first, 0.0f, firstCount);
        Adds(candidateIndices, GetTopKIndices(first, firstCapacity), 0, firstCount);
    }
    if (secondCount > 0) {
        Adds(candidateScores[secondOffset], second, 0.0f, secondCount);
        Adds(candidateIndices[secondOffset], GetTopKIndices(second, secondCapacity), 0, secondCount);
    }
    PipeBarrier<PIPE_V>();
    FinalizeTopKList(dst, inputCount, LICommon::Min(topCount, realCandidateCount), dstCapacity);
}
                                                 ...
                                                 const LIHiCachedTilingData *__restrict tilingData)
{
    ...
    localTopKRowNum_ = CeilDiv(s1BaseSize_, 2);

    groupInner_ = 16;
    blockId_ = GetBlockIdx();
    topBlockBufElems_ = LICommon::Align(
        static_cast<int32_t>(constInfo_.hiBlockNum * TOPK_LIST_COMPONENTS),
...
{
    InitTopKList(topBuf, topBlockBufElems_ / TOPK_LIST_COMPONENTS);
}
                                                                    ...
                                                                    int32_t totalBlockNum)
{
    ...
    for (int32_t pos = 0; pos < topCount; ++pos) {
        int32_t blockId = GetTopKIndex(topBuf, topBlockBufElems_ / TOPK_LIST_COMPONENTS, pos);
        if (blockId < 0) {
            continue;
        }
    ...
    for (int innerS1Idx = 0; innerS1Idx < cuS1ProcNumPerAiv; innerS1Idx++) {
        ...
        if (cuRealAcSeq > 0 && cuS2Len > 0) {
            ...
            if (useStage1ScoreTable) {
                ...
                // Long decode rows have many 128-block Stage1 tiles. Cache up
                // to BASE_TOPK raw candidates and invoke the native VF TopK
                // once per cache group instead of once per tile. Cube still
                // works on its native 128-block tile and no UB allocation grows.
                constexpr int32_t STAGE1_TILE_CACHE_COUNT = 16;
                bool useStage1TileCache = decodeStage2HiFastPathEligible_ && useStage1BlockMean;
                int32_t cacheTileIdx = useStage1TileCache ?
                ...
                LocalTensor<float> sortScoreUb;
                LocalTensor<int32_t> sortIndiceUbInt;
                if (useStage1TileCache) {
                    sortScoreUb = stage1TileCache[cacheOffset];
                    sortIndiceUbInt = stage1TileIndexCache[cacheOffset];
                } else {
                    tmpSortBuf = outQueue_.AllocTensor<float>();
                    sortScoreUb = tmpSortBuf;
                    sortIndiceUbInt = tmpSortBuf[localSortLen].template ReinterpretCast<int32_t>();
                }
                int32_t sortInitLen = useStage1TileCache ? s2BaseSize_ : localSortLen;
                ...
                if (validCount > 0) {
                    ...
                    if (vecCount > 0) {
                        ...
                        ArithProgression<int32_t>(sortIndiceUbInt[vecStart],
                                                  blockBase + vecStart, 1, vecCount);
                    }
                    for (int32_t localBlockIdx = vecEnd; localBlockIdx < validEnd; ++localBlockIdx) {
                        int32_t globalBlockId = blockBase + localBlockIdx;
                ...
                if (useStage1TileCache) {
                    bool flushTileCache = cacheTileIdx == STAGE1_TILE_CACHE_COUNT - 1 ||
                                          isS2End || info.isAllLoopEnd;
                    if (flushTileCache) {
                        int32_t groupStartTile = static_cast<int32_t>(info.s2Idx) - cacheTileIdx;
                        int32_t groupTileCount = cacheTileIdx + 1;
                        int32_t groupInputLen = groupTileCount * s2BaseSize_;
                        int32_t groupValidCount = 0;
                        for (int32_t tileIdx = 0; tileIdx < groupTileCount; ++tileIdx) {
                            int32_t tileBlockBase = (groupStartTile + tileIdx) * s2BaseSize_;
                            int32_t tileBlockCount = LICommon::Min(s2BaseSize_, totalBlockNum - tileBlockBase);
                            int32_t tileValidStart =
                                LICommon::Min(tileBlockCount, LICommon::Max(0, sinkCount - tileBlockBase));
                            int32_t tileValidEnd =
                                LICommon::Min(tileBlockCount, LICommon::Max(0, middleBlockEnd - tileBlockBase));
                            groupValidCount += LICommon::Max(tileValidEnd - tileValidStart, 0);
                        }

                        LocalTensor<float> groupTopBuf = outQueue_.AllocTensor<float>();
                        int32_t groupTopCount = LICommon::Min(groupValidCount, scoredBlockCount);
                        SelectTopKList(groupTopBuf, stage1TileCache, stage1TileIndexCache,
                                       groupInputLen, groupTopCount, BASE_TOPK);
                        int32_t rowCapacity = topBlockBufElems_ / TOPK_LIST_COMPONENTS;
                        int32_t previousValidStart =
                            LICommon::Max(static_cast<int32_t>(blockS2StartIdx_) * s2BaseSize_, sinkCount);
                        int32_t previousValidEnd =
                            LICommon::Min(groupStartTile * s2BaseSize_, middleBlockEnd);
                        int32_t previousTopCount = LICommon::Min(
                            scoredBlockCount, LICommon::Max(previousValidEnd - previousValidStart, 0));
                        if (previousTopCount == 0) {
                            CopyTopKList(rowTopBuf, rowCapacity, groupTopBuf, BASE_TOPK, groupTopCount);
                        } else {
                            MergeTopKLists(rowTopBuf, rowCapacity,
                                           rowTopBuf, rowCapacity, previousTopCount,
                                           groupTopBuf, BASE_TOPK, groupTopCount, scoredBlockCount);
                        }
                        outQueue_.FreeTensor(groupTopBuf);
                    }
                } else if (localTopCount > 0) {
                    SelectTileTopK(tmpSortBuf, sortScoreUb, sortIndiceUbInt,
                                   localSortLen, localTopCount);
                    int32_t rowCapacity = topBlockBufElems_ / TOPK_LIST_COMPONENTS;
                    int32_t localRangeStart = blockS2StartIdx_ * s2BaseSize_;
                    int32_t previousValidStart = LICommon::Max(localRangeStart, sinkCount);
                    int32_t previousValidEnd = LICommon::Min(blockBase, middleBlockEnd);
                    int32_t previousTopCount = LICommon::Min(
                        scoredBlockCount, LICommon::Max(previousValidEnd - previousValidStart, 0));
                    MergeTopKLists(rowTopBuf, rowCapacity,
                                   rowTopBuf, rowCapacity, previousTopCount,
                                   tmpSortBuf, s2BaseSize_, localTopCount, scoredBlockCount);
                }
                if (!useStage1TileCache) {
            ...
            if (useStage1ScoreTable && (needCopyOutGm || needCopyWsGm)) {
                ...
                SelectTopKList(tmpSortBuf, stage1ScoreTable, stage1IndexTable,
                               stage1BlockScoreTableLen, scoredBlockCount,
                               topBlockBufElems_ / TOPK_LIST_COMPONENTS);
                DataCopy(rowTopBuf, tmpSortBuf, topBlockBufElems_);
                PipeBarrier<PIPE_V>();
                outQueue_.FreeTensor(tmpSortBuf);
            }
            ...
            if (needCopyOutGm) {
                ...
                int32_t partialBlockStart = blockS2StartIdx_ * s2BaseSize_;
                int32_t partialBlockEnd = blockBase + localHiBlockNum;
                int32_t partialValidStart = LICommon::Max(partialBlockStart, sinkCount);
                int32_t partialValidEnd = LICommon::Min(partialBlockEnd, middleBlockEnd);
                int32_t partialTopCount = LICommon::Min(
                    scoredBlockCount, LICommon::Max(partialValidEnd - partialValidStart, 0));
...
__aicore__ inline void LIVector<LIT>::MergeStage1BlockTopM()
{
    ...
    for (uint32_t innerS1Idx = s1MergeStartIdx; innerS1Idx < s1MergeStartIdx + s1VecNum; innerS1Idx++) {
        ...
        int32_t scoredBlockCount = GetScoredBlockCount(totalBlockNum);
        int32_t topListCapacity = topBlockBufElems_ / TOPK_LIST_COMPONENTS;
        int32_t mergedCandidateCount = LICommon::Max(
        ...
        if (cuRealAcSeq > 0 && cuS2Len > 0) {
            ...
            if (info.actS1Size > 4) {
                if (hasSelectedHiTokens) {
                    ...
                    SelectTileTopK(tmpSortBuf, sortScoreUb, sortIndiceUbInt,
                                   cuS2LenVecAlign, packedHiTokenCount);
                    PipeBarrier<PIPE_V>();
                    if (isFirstStage2S2Tile) {
                        CopyTopKList(rowTopBuf, BASE_TOPK, tmpSortBuf, s2BaseSize_, s2BaseSize_);
                    } else {
                        int32_t mergeOutCount = LICommon::Min(BASE_TOPK, prevRowTopUpper + s2BaseSize_);
                        MergeTopKLists(rowTopBuf, BASE_TOPK,
                                       rowTopBuf, BASE_TOPK, prevRowTopUpper,
                                       tmpSortBuf, s2BaseSize_, s2BaseSize_, mergeOutCount);
                    }
                    PipeBarrier<PIPE_V>();
                    outQueue_.FreeTensor(tmpSortBuf);
                }
                ...
                if (globalTopkUbCacheIdx == 3 || isS2End || info.isAllLoopEnd) {
                    ...
                    if (!skipEmptyDecodeMerge) {
                        ...
                        for (int32_t listIdx = 1; listIdx < groupListCount; ++listIdx) {
                            int32_t nextCandidateCount = LICommon::Min(
                                BASE_TOPK, groupCandidateCount + s2BaseSize_);
                            MergeTopKLists(
                                tmpSortBuf, BASE_TOPK,
                                tmpSortBuf, BASE_TOPK, groupCandidateCount,
                                rowCache[listIdx * s2BaseSize_ * TOPK_LIST_COMPONENTS],
                                s2BaseSize_, s2BaseSize_, nextCandidateCount);
                            groupCandidateCount = nextCandidateCount;
                        }
                        int32_t tileIdx = static_cast<int32_t>(info.s2Idx - blockS2StartIdx_);
                        int32_t previousCandidateCount = LICommon::Min(
                            BASE_TOPK,
                        ...
                        if (previousCandidateCount == 0) {
                            ...
                            int32_t mergedCandidateCount = LICommon::Min(
                                BASE_TOPK, previousCandidateCount + groupCandidateCount);
                            MergeTopKLists(rowTopBuf, BASE_TOPK,
                                           rowTopBuf, BASE_TOPK, previousCandidateCount,
                                           tmpSortBuf, BASE_TOPK, groupCandidateCount,
...
#endif // LIGHTNING_INDEXER_HI_CACHED_ARCH35_SERVICE_VECTOR_H
