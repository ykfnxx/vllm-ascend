...
static ge::graphStatus AsuHbmIndexLookupTilingFunc(
    gert::TilingContext* context)
{
    OPS_ERR_IF(
        context == nullptr,
        OPS_LOG_E("AsuHbmIndexLookup", "TilingContext is nullptr."),
        return ge::GRAPH_FAILED);

    auto attrs = context->GetAttrs();
    OPS_LOG_E_IF_NULL(context, attrs, return ge::GRAPH_FAILED);
    const int64_t* req_num_attr =
        attrs->GetAttrPointer<int64_t>(ATTR_REQ_NUM);
    OPS_LOG_E_IF_NULL(context, req_num_attr, return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        *req_num_attr <= 0,
        OPS_LOG_E(context->GetNodeName(),
                  "req_num must be greater than 0."),
        return ge::GRAPH_FAILED);
...
static ge::graphStatus TilingParseForAsuHbmIndexLookup(
...
IMPL_OP_OPTILING(AsuHbmIndexLookup)
    ...
    .TilingParse<AsuHbmIndexLookupCompileInfo>(
        TilingParseForAsuHbmIndexLookup);
}  // namespace optiling
