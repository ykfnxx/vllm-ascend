...
public:
        ...
        TPipe* pipe)
    {
        ...
        slotToIndexGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(slotToIndex));
        freeSlotsGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(freeSlots));
        freeHeadGm_.SetGlobalBuffer(
        ...
        queryIndexGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(queryIndex),
            reqNum_ * QUERY_COUNT);
        lookupMaskGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(lookupMask),
            reqNum_ * QUERY_COUNT);
        slotOutGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(slotOut),
            reqNum_ * QUERY_COUNT);
        missOutGm_.SetGlobalBuffer(
            reinterpret_cast<__gm__ int32_t*>(missOut),
            reqNum_ * QUERY_COUNT);

        pipe_->InitBuffer(
            queryBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            lookupMaskInputBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            indexBuf_, INDEX_TILE_LEN * sizeof(int32_t));
        pipe_->InitBuffer(
            outBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            missBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            candidateBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            deltaBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            clampBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            helperBuf_, QUERY_COUNT * sizeof(int32_t));
        pipe_->InitBuffer(
            offsetBuf_, QUERY_COUNT * sizeof(uint32_t));
        pipe_->InitBuffer(
            maskBuf_, QUERY_COUNT * sizeof(uint8_t));
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        auto lookupMaskInputTile =
            lookupMaskInputBuf_.Get<int32_t>();
        ...
        for (uint32_t reqId = coreId;
             reqId < reqNum_;
             reqId += blockNum) {
            uint32_t poolEntry =
                static_cast<uint32_t>(
                    reqPoolEntriesGm_.GetValue(reqId));
            ...
            uint32_t freeHeadOffset =
                poolEntry * FREE_HEAD_STRIDE;
            uint32_t queryReqBase = reqId * QUERY_COUNT;
            int32_t freeHead =
                freeHeadGm_.GetValue(freeHeadOffset);

            DataCopy(
                queryTile,
                queryIndexGm_[queryReqBase],
                QUERY_COUNT);
            DataCopy(
                lookupMaskInputTile,
                lookupMaskGm_[queryReqBase],
                QUERY_COUNT);
            SyncPipelines<HardEvent::MTE2_V>();
            Duplicate(outTile, NOT_FOUND, QUERY_COUNT);
            Duplicate(
                missTile, static_cast<int32_t>(0), QUERY_COUNT);
            PipeBarrier<PIPE_ALL>();

            for (uint32_t indexBase = 0;
                 indexBase < INDEX_SIZE;
                 indexBase += INDEX_TILE_LEN) {
                if (indexBase != 0U) {
                    SyncPipelines<HardEvent::V_MTE2>();
                }
                DataCopy(
                    indexTile,
                    indexGm_[indexReqBase + indexBase],
                    INDEX_TILE_LEN);
                SyncPipelines<HardEvent::MTE2_V>();
            ...
            for (uint32_t i = 0; i < QUERY_COUNT; ++i) {
                ...
                if (slot == NOT_FOUND) {
                    ...
                    slot = indexGm_.GetValue(
                        indexReqBase
                        + static_cast<uint32_t>(indexId));
                    if (slot == NOT_FOUND) {
                        slot = freeSlotsGm_.GetValue(
                            freeReqBase
                            + static_cast<uint32_t>(freeHead));
                        ++freeHead;
                        indexGm_.SetValue(
                            indexReqBase
                                + static_cast<uint32_t>(indexId),
            ...
            DataCopy(
                slotOutGm_[queryReqBase],
                outTile,
                QUERY_COUNT);
            DataCopy(
                missOutGm_[queryReqBase],
                missTile,
                QUERY_COUNT);
...
private:
    ...
    {
        event_t eventId =
            static_cast<event_t>(pipe_->FetchEventID(event));
...
extern "C" __global__ __aicore__ void asu_hbm_index_lookup(
    GM_ADDR index,
    GM_ADDR slotToIndex,
    GM_ADDR freeSlots,
    GM_ADDR freeHead,
