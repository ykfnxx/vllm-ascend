#!/usr/bin/env python3
"""Run turbo_prefetch + turbo_fused_prefetch A5 lookup for one CSV case, for
msprof per-op timing.

Mirror of the turbo baseline's msprof_case.py: one workload (batch /
index_capacity / hit_rate / qpr) launches BOTH
dsa_sparse_turbo_prefetch_lookup_update_batch (baseline) and
dsa_sparse_turbo_fused_prefetch_lookup_update_batch (V2, in-kernel history
classification + destination mapping), interleaved, so a wrapping `msprof`
produces one op_summary row per kernel launch distinguishable by Op Name.
...
def main():
    ...
    for _ in range(20):
        torch.ops._C_ascend.dsa_sparse_turbo_prefetch_lookup_update_batch(*turbo_args())
    for _ in range(20):
        torch.ops._C_ascend.dsa_sparse_turbo_fused_prefetch_lookup_update_batch(*fused_args())
