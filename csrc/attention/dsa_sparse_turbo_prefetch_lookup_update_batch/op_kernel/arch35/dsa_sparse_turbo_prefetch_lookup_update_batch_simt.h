...
#ifndef DSA_SPARSE_LOOKUP_UPDATE_TURBO_PREFETCH_SIMT_H
#define DSA_SPARSE_LOOKUP_UPDATE_TURBO_PREFETCH_SIMT_H

#include "../dsa_sparse_turbo_prefetch_lookup_update_batch_common.h"
...
namespace DsaSparseTurboPrefetchLookupUpdateBatch {
...
// Plan-2 refill rank counter (UB atomic): threads grab a refill rank while
// scanning for victims; the count after the eviction pass is the actual
// number of replenished slots (no victim-count pass needed).
...
// Prefetch-only request-level maintain (coarse design_and_test.md §9, plans 2
// and 3).  Plan 2: the victim-count pass is GONE — victims are found in a
// single eviction pass whose refill ranks are grabbed with a UB atomic
// counter (asc_atomic_add, ~10-30ns — the GM-atomic lesson from coarse
// optimization A does not apply), so the scan stops once `cum_alloc` victims
// are found.  Plan 3: the scan covers ONLY the resident region [0, 8192)
// (cursor mapped into the resident ring); free-region slots [8192, 10240) are
// never scanned, so stale mappings there are a TEMPORARY leak reclaimed by
// the exact path's full-scan maintain — safe only because this operator is
// used exclusively by the prefetch stream (L+1 exact lookup re-derives).
// Kept from the turbo kernel: per-request consolidation, the early return
// when nothing was allocated, the overflow fallback (alloc ledger revert
// when the atomic eviction falls short), and the Plan-1 crash-safety marker.
__simt_callee__ inline void PrefetchMaintain(
    __gm__ int32_t* request_index,
    __gm__ int32_t* request_slot_to_index,
    __gm__ int32_t* request_free_slots,
    ...
    uint32_t index_capacity)
{
    ...
    constexpr uint32_t resident_chunk =
        DSA_SPARSE_TURBO_RESIDENT_SLOT_COUNT /
        DSA_SPARSE_TURBO_SIMT_THREADS;
    ...
    for (uint32_t position = scan_begin;
        ...
        if (slot >= DSA_SPARSE_TURBO_RESIDENT_SLOT_COUNT) {
            slot -= DSA_SPARSE_TURBO_RESIDENT_SLOT_COUNT;
        }
    ...
    // index/slot_to_index reset and outputs back to the fallback value.
    // Their free-list entries were never consumed by the refill, so the free
    // list self-heals without extra work.  Runs only on the rare
    // victim-shortage path.
...
DsaSparseTurboPrefetchLookupUpdateBatchSimt(
    __gm__ int32_t* index,
    __gm__ int32_t* slot_to_index,
    __gm__ int32_t* free_slots,
    __gm__ int32_t* free_head,
...
#endif  // DSA_SPARSE_LOOKUP_UPDATE_TURBO_PREFETCH_SIMT_H
