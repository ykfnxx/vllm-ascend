...
#include "arch35/dsa_sparse_lookup_update_batch_simt.h"
#include "dsa_sparse_lookup_update_batch_common.h"

extern "C" __global__ __aicore__ void dsa_sparse_lookup_update_batch(
    GM_ADDR index,
    ...
    GM_ADDR tiling)
{
    ...
    REGISTER_TILING_DEFAULT(DsaSparseLookupUpdateBatchTilingData);
    GET_TILING_DATA_WITH_STRUCT(
        DsaSparseLookupUpdateBatchTilingData,
        tiling_data,
        tiling);

    __ubuf__ uint32_t
        shared_scratch[DSA_SPARSE_BATCH_UB_SCRATCH_WORDS];
    asc_vf_call<
        DsaSparseLookupUpdateBatch::DsaSparseLookupUpdateBatchSimt>(
        dim3(DSA_SPARSE_BATCH_SIMT_THREADS),
