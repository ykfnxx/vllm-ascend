...
#ifndef DSA_SPARSE_LOOKUP_UPDATE_BATCH_SIMT_H
#define DSA_SPARSE_LOOKUP_UPDATE_BATCH_SIMT_H

#include "../dsa_sparse_lookup_update_batch_common.h"
...
namespace DsaSparseLookupUpdateBatch {
    ...
    __ubuf__ int32_t* warp_totals)
{
    ...
    const uint32_t lane =
        tid & (DSA_SPARSE_BATCH_WARP_SIZE - 1U);
    const uint32_t warp = tid / DSA_SPARSE_BATCH_WARP_SIZE;

    int32_t inclusive = value;
    for (uint32_t delta = 1U;
         delta < DSA_SPARSE_BATCH_WARP_SIZE;
         delta <<= 1U) {
    ...
    if (lane == DSA_SPARSE_BATCH_WARP_SIZE - 1U) {
    ...
    if (warp == 0U) {
        int32_t warp_inclusive =
            lane < DSA_SPARSE_BATCH_WARP_COUNT
                ? warp_totals[lane]
                : 0;
        for (uint32_t delta = 1U;
             delta < DSA_SPARSE_BATCH_WARP_SIZE;
        ...
        if (lane < DSA_SPARSE_BATCH_WARP_COUNT) {
    ...
    uint32_t query_end)
{
    ...
    constexpr uint32_t query_chunk =
        DSA_SPARSE_BATCH_QUERY_WIDTH /
        DSA_SPARSE_BATCH_SIMT_THREADS;
        ...
        for (uint32_t local_entry = 0U;
            ...
            const bool active =
                ...
                token < static_cast<int32_t>(
                            DSA_SPARSE_BATCH_INDEX_CAPACITY);
            slot_out[offset] =
                active ? DSA_SPARSE_BATCH_FALLBACK_SLOT
                       : DSA_SPARSE_BATCH_NOT_FOUND;
            miss_out[offset] = 0;
        }
    }
}
    ...
    for (uint32_t local_entry = 0U;
        ...
        local_slots[local_entry] =
            mask != 0 && token >= 0 &&
                    token < static_cast<int32_t>(
                                DSA_SPARSE_BATCH_INDEX_CAPACITY)
                ? DSA_SPARSE_BATCH_FALLBACK_SLOT
                : DSA_SPARSE_BATCH_NOT_FOUND;
...
#pragma unroll
    ...
    const int32_t total_misses =
        warp_totals[DSA_SPARSE_BATCH_WARP_COUNT - 1U];
    ...
    for (uint32_t position = scan_begin;
        ...
        if (slot >= DSA_SPARSE_BATCH_SLOT_COUNT) {
            slot -= DSA_SPARSE_BATCH_SLOT_COUNT;
        }
    ...
    if (tid == 0U) {
        ...
        if (safe_alloc > static_cast<int32_t>(
                             DSA_SPARSE_BATCH_FREE_SLOT_COUNT)) {
            safe_alloc = static_cast<int32_t>(
                DSA_SPARSE_BATCH_FREE_SLOT_COUNT);
        }
        scalars[kSafeAllocScalar] = safe_alloc;
        scalars[kLastVictimScalar] = DSA_SPARSE_BATCH_NOT_FOUND;
    }
    asc_syncthreads();
    ...
    if (victim_prefix < safe_alloc) {
        for (uint32_t position = scan_begin;
            ...
            const int32_t old_token =
                request_slot_to_index[slot];
            if (old_token == DSA_SPARSE_BATCH_NOT_FOUND) {
                continue;
            }
    ...
    bool reuse_scratch)
{
    ...
    __ubuf__ int32_t* warp_totals =
        reinterpret_cast<__ubuf__ int32_t*>(
            protected_bits + DSA_SPARSE_BATCH_PROTECTED_WORDS);
    __ubuf__ int32_t* scalars =
        warp_totals + DSA_SPARSE_BATCH_WARP_COUNT;

    for (uint32_t word = tid;
         word < DSA_SPARSE_BATCH_PROTECTED_WORDS;
         word += thread_count) {
        ...
        if (pool_entry >= 0 &&
            ...
            __gm__ int32_t* request_free_head =
                free_head + static_cast<uint64_t>(pool_entry) *
                                DSA_SPARSE_BATCH_FREE_HEAD_STRIDE;
...
__simt_vf__ __launch_bounds__(DSA_SPARSE_BATCH_SIMT_THREADS) inline void
DsaSparseLookupUpdateBatchSimt(
    __gm__ int32_t* index,
    __gm__ int32_t* slot_to_index,
    __gm__ int32_t* free_slots,
    __gm__ int32_t* free_head,
...
#endif  // DSA_SPARSE_LOOKUP_UPDATE_BATCH_SIMT_H
