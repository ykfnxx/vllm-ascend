...
#include "dsa_sparse_lookup_update_batch_tiling.h"
...
static ge::graphStatus DsaSparseLookupUpdateBatchTilingFunc(
    gert::TilingContext* context)
{
    if (context == nullptr) {
    ...
    auto* tiling_data =
        context->GetTilingData<DsaSparseLookupUpdateBatchTilingData>();
...
static ge::graphStatus TilingParseForDsaSparseLookupUpdateBatch(
...
IMPL_OP_OPTILING(DsaSparseLookupUpdateBatch)
    .Tiling(DsaSparseLookupUpdateBatchTilingFunc)
    .TilingParse<DsaSparseLookupUpdateBatchCompileInfo>(
        TilingParseForDsaSparseLookupUpdateBatch);
