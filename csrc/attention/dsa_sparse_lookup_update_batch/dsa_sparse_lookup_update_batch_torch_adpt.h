...
#ifndef DSA_SPARSE_LOOKUP_UPDATE_BATCH_TORCH_ADPT_H
#define DSA_SPARSE_LOOKUP_UPDATE_BATCH_TORCH_ADPT_H
    ...
    int64_t req_num)
{
    ...
    TORCH_CHECK(index.is_contiguous() &&
                ...
                "all dsa_sparse_lookup_update_batch tensors must be contiguous");
    ...
    TORCH_CHECK(slot_to_index.device() == device &&
                ...
                "all dsa_sparse_lookup_update_batch tensors must be on one device");
...
#endif  // DSA_SPARSE_LOOKUP_UPDATE_BATCH_TORCH_ADPT_H
