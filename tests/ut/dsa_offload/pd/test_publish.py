...
def test_final_prefill_puts_full_blocks_and_captures_last_topk_and_tail(spy_io) -> None:
    ...
    state = PrefillPublishState(
        ...
        committed_block_keys={"final": [101], "middle": [102]},
        io_backend=spy_io,
        tp_rank=1,
    )
    ...
    assert spy_io.put_calls[0]["storage_ids"].tolist() == [
        make_storage_id(101, 7)
    ]
...
def test_full_block_publish_rejects_missing_keys(spy_io) -> None:
    state = PrefillPublishState(
        request_ids=("request",),
    ...
    with pytest.raises(
        RuntimeError,
        match=r"Prefill publish for request request requires 1 block keys",
    ):
        state.publish_layer(
            layer_name="layer",
            layer_id=0,
