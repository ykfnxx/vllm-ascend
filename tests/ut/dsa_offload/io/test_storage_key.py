...
def test_storage_id_is_stable_positive_int63() -> None:
    ...
    assert storage_id != make_storage_id(block_hash + b"next", 7)
    assert 0 < storage_id < 1 << 63


def test_storage_id_uses_block_hash_and_physical_layer() -> None:
    first = b"first"
    second = b"second"
...
def test_storage_ids_builds_int64_tensor_on_requested_device() -> None:
    block_hashes = [b"block-0", b"block-1"]

    storage_ids = make_storage_ids(block_hashes, 2, device="cpu")

    assert storage_ids.dtype == torch.int64
    assert storage_ids.device.type == "cpu"
    assert storage_ids.tolist() == [
        make_storage_id(block_hash, 2) for block_hash in block_hashes
    ]
